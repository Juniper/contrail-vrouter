...
//Number of Logs that can be logged
#define VR_LOG_ENTRY_LEN 100

//Max size of log buffer
#define VR_LOG_MAX 3000
...
//Log API
#define VR_LOG(idx, log, mod, lev, fmt, ...) \
{ \
	...
	if(level[mod] & LOG_CON_MASK) \
	    ...
	    vr_log->vr_log_buf[mod].buf_idx = (vr_log->vr_log_buf[mod].buf_idx + length)%VR_LOG_MAX;\
	}\
    } \
}

#define MOD_LOG(A, B, C) vr_log_mod_func(A, B, C)



struct vrouter* find_vrouter(unsigned int rid);
