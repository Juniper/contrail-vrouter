...
#			 1: dump vif with vif_idx wiht all details)
define dump_vif_internal
	if ($arg0)
		if(router.vr_interfaces[$arg0])
			set $cur_vif = (router.vr_interfaces[$arg0])
			set $int_ip = $cur_vif.vif_ip
                        printf "\nvif 0/%-6uName:%s IPaddr:%d.%d.%d.%d HWaddr:", $cur_vif.vif_idx, $cur_vif.vif_name, ($int_ip & 0xff), ($int_ip >> 8) & 0xff, ($int_ip >> 16) & 0xff, ($int_ip >> 24)  & 0xff
	                mac_address $cur_vif.vif_mac
			printf "GenNum:%u", $cur_vif.vif_gen
			printf "\n            Type:"
			get_vif_type (int)$cur_vif.vif_type
			printf " VLanID:%u OVlanID:%u Router:%#x, Users:%u", $cur_vif.vif_vlan_id, $cur_vif.vif_ovlan_id, $cur_vif.vif_router, $cur_vif.vif_users
			printf "\n            Rid:%u NHid:%u Vrf:%u Mcast Vrf:%u MTU:%u Flags:", $cur_vif.vif_rid, $cur_vif.vif_nh_id, $cur_vif.vif_vrf, $cur_vif.vif_mcast_vrf,$cur_vif.vif_mtu
			get_vif_flags $cur_vif.vif_flags
			printf " OS_Id:%u\n", $cur_vif.vif_os_idx
			printf"            "
			get_vif_transport $cur_vif.vif_transport

			printf " QOS:%d", $cur_vif.vif_qos_map_index
			get_vhostuser_mode $cur_vif.vif_vhostuser_mode
			printf " vrfTableUsers:%u\n            IPv6:", $cur_vif.vif_vrf_table_users
			...
			get_hw_queues $cur_vif.vif_hw_queues
...
#------------------------
			...
			if ($arg1)
				if ($cur_vif.vif_vrf_table)
					get_vrf_table $curf_vif.vif_vrf_table
				end

				if ($cur_vif.vif_sub_interfaces)
					get_sub_interfaces $cur_vif.vif_sub_interfaces
				end
			end
			printf "\n"
		end
...
#arg0:uint8_t *mac
define mac_address
		printf "%02x:%02x:%02x:%02x:%02x:%02x ", $arg0[0], $arg0[1], $arg0[2], $arg0[3], $arg0[4], $arg0[5]
end

#arg0:vif_transport of cur_vif
define get_vif_transport
	printf "Transport:"
	if($arg0 == 0)
		printf "VIRTUAL"
	end
...
define get_vif_stats
	...
	printf "                     Input: "
	printf "ibytes:%llu ipackets:%llu ierrors:%llu\n", $arg0.vis_ibytes, $arg0.vis_ipackets, $arg0.vis_ierrors
        printf "                    Output: "
        printf "obytes:%llu opackets:%llu oerrors:%llu\n", $arg0.vis_obytes, $arg0.vis_opackets, $arg0.vis_oerrors
        printf "                     Queue: "        
        printf "ipackets:%llu ierrors:%llu opackets:%llu oerrors:%llu\n", $arg0.vis_queue_ipackets, $arg0.vis_queue_ierrors, $arg0.vis_queue_opackets, $arg0.vis_queue_oerrors
	...
	printf"\n"
	printf "                      Port: "
        printf "ipackets:%llu ierrors:%llu isyscalls:%llu inombufs:%llu\n                            opackets:%llu oerrors:%llu osyscalls:%llu\n", $arg0.vis_port_ipackets, $arg0.vis_port_ierrors, $arg0.vis_port_isyscalls, $arg0.vis_port_inombufs, $arg0.vis_port_opackets, $arg0.vis_port_oerrors, $arg0.vis_port_osyscalls
        printf "                       Dev: "        
        printf "ibytes:%llu ipackets:%llu ierrors:%llu inombufs:%llu\n                            obytes:%llu opackets:%llu oerrors:%llu\n", $arg0.vis_dev_ibytes, $arg0.vis_dev_ipackets, $arg0.vis_dev_ierrors, $arg0.vis_dev_inombufs, $arg0.vis_dev_obytes, $arg0.vis_dev_opackets, $arg0.vis_dev_oerrors
end

#arg0:vif_vhostuser_mode of cur_vif
define get_vhostuser_mode
...
define get_fat_flow_config
	...
	while($count < $arg0)
		...
		printf "                          %d: ProtocolNo:%u PortNo:%u\n", $count, $cur_cfg.protocol, $cur_cfg.port
		set $aggr_info_val1 = ($cur_cfg.port_aggr_info & 0x03)
		set $arrg_info_val2 = ($cur_cfg.port_aggr_info & 0xF0) >> 4
		printf "                             PortAggrInfo:"
		...
		if ($cur_cfg.src_prefix_h)
			...
			printf "%d.%d.%d.%d", $s_ip & 0xff, ($s_ip >> 8) & 0xff, ($s_ip >> 16) & 0xff, ($s_ip >> 24) & 0xff
		end
		printf "\n                             PrefixMask:%u AggrLen:%u\n", $cur_cfg.src_prefix_mask, $cur_cfg.src_aggregate_plen
		...
		printf "                             DstPrefixInfo:\n                             IPAddr:"
                ...
                printf "                             PrefixMask:%u AggrLen:%u\n", $cur_cfg.dst_prefix_mask, $cur_cfg.dst_aggregate_plen
...
define get_fat_flow_exclude_list_ipv4
	if ($arg0)
		...
		while($count < $arg0)
			set $ip = $arg2[$count]
			printf "            %d.%d.%d.%d/%u\n", ($ip & 0xff), ($ip >> 8) & 0xff, ($ip >> 16) & 0xff, ($ip & 24) & 0xff, $arg1[$count++]
		end
	end
end
...
define sum_pcpu_stat
	...
	if($arg0)
		while($cpu<vr_num_cpus)
			...
			get_index_addr_btable $arg0 $index
			if($bt_ret_val != -1)
				set $sum = $sum + (*(uint8_t *)$bt_ret_val)
			end
			#printf "\n%u", $sum
			set $cpu = $cpu + 1
		end
