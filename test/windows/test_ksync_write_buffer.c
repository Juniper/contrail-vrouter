...
void Test_WinKsyncWriteBuffer_parse_write(void **state) {
    ...
    assert_int_equal(rsi.Status, STATUS_SUCCESS);
}
...
void Test_WinKsyncWriteBuffer_parse_write_2msgs(void **state) {
    ...
    assert_int_equal(rsi.Information, NHL_SIZE * 2 + strlen("abcd" "xxxxxxx"));
    assert_int_equal(rsi.Status, STATUS_SUCCESS);
}
...
void Test_WinKsyncWriteBuffer_parse_write_overhead(void **state) {
    ...
    assert_int_equal(rsi.Information, ctx->WrittenBytes);
    assert_int_equal(rsi.Status, STATUS_UNSUCCESSFUL);
}
