...
Test_win_tx_pp_ArpPacket(void **state)
{
    ...
    packet->vp_if = test_calloc(1, sizeof(*packet->vp_if));
...
Test_win_tx_pp_SmallIpUdpOverTunnelPacket(void **state)
{
    ...
    Fake_WinSubPacketSetData(subPkt, buffer, 84);
    ...
    // NOTE: Cache value, because we need to release vrPacket before any assert fails.
    //       It is needed, because cmocka is buggy on Windows.
    ...
    assert_int_equal(innerIpCsum, 0xd375);
    assert_int_equal(innerUdpCsum, 0x0534);
    //assert_true(WinPacketRawShouldIpChecksumBeOffloaded(rawPkt));
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(Test_win_tx_pp_SmallIpUdpOverTunnelPacket),
    };
