...
void
Test_segmentation_ReturnsCorrectCsum1(void **state)
{
    struct SplittingContext pctx;
    struct vr_ip ip_header;

    pctx.inner_eth_header_size = 20;
    pctx.inner_headers_size = 60;
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(Test_segmentation_ReturnsCorrectCsum1),
    };
