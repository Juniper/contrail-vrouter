...
_vrouter_get_nexthop_creating_new_flow(struct vrouter *router,
        unsigned int index)
{
    struct vr_flow flow;
    flow_result_t result;
...
test_new_flow_concurrency(void **state)
{
    ...
    /* change the action now */
    fe = vr_find_flow(router, &flow, VP_TYPE_IP, &(flow_req.fr_index));
    assert_non_null(fe);
...
setup_flow(void **state)
{
    ...
    if (!router->vr_agent_if) {
         agent_vif = (struct vr_interface *)
             malloc(sizeof(struct vr_interface));
