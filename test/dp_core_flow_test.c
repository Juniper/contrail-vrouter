...
static void
_fill_fake_flow(const char *sip, const char *dip,
        unsigned short sport, unsigned short dport, unsigned short nh_id,
        unsigned char proto, struct vr_flow *flow)
{
    struct sockaddr_in sip_sin, dip_sin;

    bzero(flow, sizeof(struct vr_flow));
    inet_aton(sip, &sip_sin.sin_addr);
    inet_aton(dip, &dip_sin.sin_addr);

    vr_inet_fill_flow(flow, nh_id, sip_sin.sin_addr.s_addr,
            dip_sin.sin_addr.s_addr, proto, htons(sport),
            htons(dport));
}
...
_fill_flow_req(vr_flow_req *flow_req, struct vr_flow *flow)
{
    ...
    flow_req->fr_flow_sip = flow->flow4_sip;
    flow_req->fr_flow_dip = flow->flow4_dip;
...
test_new_flow_from_agent(void **state)
{
    ...
    vr_inet_fill_flow(&flow, NEXTHOP_1,
