...
class TestTcpReset(unittest.TestCase):
    ...
    @classmethod
    def setup_class(cls, dpdk_args=None):
        ObjectBase.setUpClass(dpdk_args)
    ...
    def test_tcp_reset_0(self):
        ...
        # Setting vrouter_instance for next testcase
        os.system('kill -9 $(pidof contrail-vrouter-dpdk)')

        os.environ["VTEST_ONLY_MODE"] = "0"
        dpdk_args = ["--vr_close_flow_on_tcp_rst", "1"]
        self.setup_class(dpdk_args)
	os.environ["VTEST_ONLY_MODE"] = "0"


    def test_tcp_rst_1(self):
        rst_pkt = self.establish_flow()
