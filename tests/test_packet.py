...
class TestPacket(unittest.TestCase, vtest_common.VTestCommon):
    ...
    def test_icmp_packet(self):
        icmp = IcmpPacket(sip='1.1.1.4', dip='2.2.2.4', 
                            smac='02:88:67:0c:2e:11', dmac='00:00:5e:00:01:00', id=1136)
        pkt = icmp.get_packet()
        self.assertIsNotNone(pkt)
        pkt.show() 
    ...
    def test_mpls_over_udp_packet(self):
        ...
        mpls = MplsoUdpPacket(label=42, sip='8.0.0.3', dip='8.0.0.2',
                            smac="de:ad:be:ef:00:02", dmac="de:ad:be:ef:00:00",
                            sport=53363, dport=6635, inner_pkt=pkt) 
        pkt = mpls.get_packet()
        pkt.show()
        self.assertIsNotNone(pkt)

        icmp_inner = IcmpPacket(sip='2.2.2.3', dip='1.1.1.3', 
                    smac='c0:d2:00:06:44:7c', dmac='02:c2:23:4c:d0:55', icmp_type=0)
        pkt = icmp_inner.get_packet()
        self.assertIsNotNone(pkt)
    ...
    def test_mpls_over_gre_packet(self):
        ...
        mpls = MplsoGrePacket(label=42, sip='8.0.0.3', dip='8.0.0.2',
                            smac="de:ad:be:ef:00:02", dmac="de:ad:be:ef:00:00",
                            inner_pkt=pkt) 
        pkt = mpls.get_packet()
        self.assertIsNotNone(pkt)
