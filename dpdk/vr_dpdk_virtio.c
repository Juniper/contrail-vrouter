                                                    ...
                                                    char* pkt_addr, uint32_t pkt_len)
{
    ...
    while (pktlen_to_copy > 0) {
        ...
        copy_len = len - last_mbuf->data_len;
...
dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts)
{
    ...
    for (i = 0; i < avail_pkts; i++) {
        ...
        if (mbuf->ol_flags & PKT_RX_GSO_TCP4)
        ...
        if (unlikely(mbuf->tso_segsz == 0)) {
            ...
            if (unlikely(dpdk_virtio_create_chained_len_sized_mbuf(mbuf, mbuf->tso_segsz, pkt_addr, pkt_len) < 0)) {
                goto free_mbuf;
            }
