...
dpdk_virtio_get_ip_tcp_hdr_len(char *pkt_addr, uint32_t pkt_len)
{
    ...
    if (likely(eth_proto == rte_cpu_to_be_16(VR_ETH_PROTO_IP))) {
        ...
        pull_len += ipv4_hlen;
        tcp_hdr = (struct vr_tcp*)(ipv4_hdr + ipv4_hlen);
...
/*
 * dpdk_virtio_create_chained_len_sized_mbuf - Create a chained mbuf of given lenght 
 * and copy the data pointed to by pkt_addr of len pkt_len
 */
static int
dpdk_virtio_create_chained_len_sized_mbuf(struct rte_mbuf *mbuf, uint32_t len, 
                               char* pkt_addr, uint32_t pkt_len, uint32_t header_len)
{
    ...
    while (pktlen_to_copy > 0) {
        //copy_len = len - rte_pktmbuf_lastseg(mbuf)->data_len;
        copy_len = len + header_len - last_mbuf->data_len;
        header_len = 0;
...
dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts)
{
    ...
    for (i = 0; i < avail_pkts; i++) {
        ...
        if (unlikely(mbuf->tso_segsz == 0)) {
            ...
            if (unlikely(dpdk_virtio_create_chained_len_sized_mbuf(mbuf, 
    ...
    uint16_t res_end_idx, struct rte_mbuf *pkt)
{
    ...
    vb_addr = (uintptr_t)vr_dpdk_guest_phys_to_host_virt(vru_cl, 
                                                        vq->vdv_buf_vec[vec_idx].buf_addr);
    vb_hdr_addr = vb_addr;
    ...
    vb_avail = vq->vdv_buf_vec[vec_idx].buf_len - vq->vdv_hlen;

    entry_len = vq->vdv_hlen;

    if (vb_avail == 0) {
        uint32_t desc_idx =
            vq->vdv_buf_vec[vec_idx].desc_idx;
        ...
        rte_prefetch0((void *)(uintptr_t)vb_addr);
        vb_offset = 0;
        vb_avail = vq->vdv_buf_vec[vec_idx].buf_len;
    }
    ...
    while (cpy_len > 0) {
        ...
        if (seg_avail != 0) {
             ...
             */
            if ((vq->vdv_desc[vq->vdv_buf_vec[vec_idx].desc_idx].flags &
                VRING_DESC_F_NEXT) == 0) {
                /* Update vdv_used ring with vdv_desc information */
                vq->vdv_used->ring[cur_idx & (vq->vdv_size - 1)].id
                    = vq->vdv_buf_vec[vec_idx].desc_idx;
            ...
            if (pkt != NULL) {
                ...
                if (vb_avail == 0) {
                    ...
                    vb_addr = (uintptr_t)vr_dpdk_guest_phys_to_host_virt(vru_cl,
                                                    vq->vdv_buf_vec[vec_idx].buf_addr);
                    vb_avail =
                        vq->vdv_buf_vec[vec_idx].buf_len;
                    vb_offset = 0;
                }
    ...
    uint32_t *secure_len, uint32_t *vec_idx)
{
    ...
    do {
        ...
        vq->vdv_buf_vec[vec_id].buf_addr = vq->vdv_desc[idx].addr;
        vq->vdv_buf_vec[vec_id].buf_len = vq->vdv_desc[idx].len;
        vq->vdv_buf_vec[vec_id].desc_idx = idx;
        vec_id++;
                               ...
                               unsigned int *vring_basep)
{
    ...
    *vring_basep = vq->vdv_last_used_idx;
     ...
     * state to indicate that this queue should not be vdv_used any more.
     */
 ...
 * available/vdv_used rings based on messages sent by the vhost client.
 *
 * Returns 0 on success, -1 otherwise.
 */
int
...
/*
 * vr_dpdk_set_ring_callfd - set the eventd vdv_used to raise interrupts in
 ...
 * vdv_used to store a pointer to the vhost client structure.
 *
 * Returns nothing.
 */
void
