...
dpdk_virtio_vq_get(unsigned int vif_idx, unsigned int vring_idx)
{
...
static inline bool
dpdk_virtio_is_ready(vr_dpdk_virtioq_t *vq)
{
    rte_rmb();
...
/*
 * vr_dpdk_virtio_is_ready - check virtio queue ready state.
 *
 * Returns true if virtio queue is in ready state, false otherwise.
 */
bool
vr_dpdk_virtio_is_ready(unsigned int vif_idx, unsigned int vring_idx)
{
    vr_dpdk_virtioq_t *vq = dpdk_virtio_vq_get(vif_idx, vring_idx);

    if (!vq)
        return false;

    return dpdk_virtio_is_ready(vq);
}

/*
 * vr_dpdk_virtio_can_start - check if virtio queue can be started.
 *
 * Returns true if virtio queue is ready to start, false otherwise.
 */
bool
vr_dpdk_virtio_can_start(unsigned int vif_idx, unsigned int vring_idx)
{
    vr_dpdk_virtioq_t *vq = dpdk_virtio_vq_get(vif_idx, vring_idx);
    ...
    return vq->vdv_desc && vq->vdv_avail && vq->vdv_used && vq->vdv_size;
}

static int
dpdk_virtio_reset(vr_dpdk_virtioq_t *vq)
{
                             ...
                             unsigned int queue_or_lcore_id)
{
    ...
    /* init virtio queue */
    dpdk_virtio_reset(&vr_dpdk_virtio_rxqs[vif_idx][queue_id]);
                             ...
                             unsigned int queue_or_lcore_id)
{
    ...
    /* init virtio queue */
    dpdk_virtio_reset(&vr_dpdk_virtio_txqs[vif_idx][queue_id]);
...
dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts)
{
    ...
    if (unlikely(!dpdk_virtio_is_ready(vq))) {
                               ...
                               unsigned int *vring_basep)
{
    ...
    *vring_basep = vq->vdv_last_used_idx;
     ...
     */
    vr_dpdk_virtio_ready_set(vif_idx, vring_idx, VQ_NOT_READY);
...
/*
 * vr_dpdk_virtio_ready_set - sets the virtio queue ready state to indicate
 * whether forwarding can start on the virtio queue or not.
 *
 * Returns 0 on success, -1 otherwise.
 */
int
vr_dpdk_virtio_ready_set(unsigned int vif_idx, unsigned int vring_idx,
                        vq_ready_state_t ready)
{
    ...
    rte_wmb();
    if (ready == VQ_NOT_READY) {
        synchronize_rcu();
        /* Close FDs and reset the queue. */
        dpdk_virtio_reset(vq);
    } else {
        ...
        if (vq->vdv_used) {
            ...
            if (vq->vdv_last_used_idx != vq->vdv_used->idx) {
                ...
                return vr_dpdk_virtio_set_vring_base(vif_idx, vring_idx,
                        vq->vdv_used->idx);
            }
        }
    }
