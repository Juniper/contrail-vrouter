...
dpdk_virtio_send_burst(struct dpdk_virtio_writer *p)
{
    ...
    if (unlikely(p->tx_virtioq->vdv_send_func == NULL)) {
        struct vr_interface *vif;
        vif = __vrouter_get_interface(vrouter_get(0), p->tx_virtioq->vdv_vif_idx);
        if (vif && (vif->vif_flags & VIF_FLAG_GRO_NEEDED))
            p->tx_virtioq->vdv_send_func = dpdk_virtio_dev_to_vm_tx_burst_mergeable; 
        else
            p->tx_virtioq->vdv_send_func = dpdk_virtio_dev_to_vm_tx_burst; 
    }
