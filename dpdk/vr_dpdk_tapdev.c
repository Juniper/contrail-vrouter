...
vr_dpdk_tapdev_init(struct vr_interface *vif)
{
    ...
    /* If it's a VLAN device, assign vhost0 MAC address to it */
    if (vif->vif_type == VIF_TYPE_VLAN) {
        struct vrouter *router = vrouter_get(0);
        rte_memcpy(ifr.ifr_hwaddr.sa_data, router->vr_host_if->vif_mac, ETHER_ADDR_LEN);
        if (ioctl(fd, SIOCSIFHWADDR, &ifr) < 0) {
            RTE_LOG(ERR, VROUTER, "    error assigning MAC address\n");
        }
    }
