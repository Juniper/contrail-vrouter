...
void vr_dpdk_tapdev_handle_notifications(void)
{
         ...
         h = NLMSG_NEXT (h, status))
    {
        ...
        if (h->nlmsg_type == RTM_NEWLINK) {
            ...
            ifi = NLMSG_DATA (h);
            RTE_LOG(INFO, VROUTER, "Netlink notification received\n");
                                           ...
                                           attribute = RTA_NEXT(attribute, len))
            {
                switch(attribute->rta_type) {
                    case IFLA_IFNAME:
                        ...
                        RTE_LOG(INFO, VROUTER, "Interface %d : %s\n",
                                           ifi->ifi_index, ifname);
                        break;
                    case IFLA_MTU:
                        mtu = *(uint32_t*) RTA_DATA(attribute);
                        RTE_LOG(INFO, VROUTER, "MTU %d : %d\n",
                                           ifi->ifi_index, mtu);
                        break;
                    default:
                        break;
            ...
            if (ifname && (strncmp(ifname, VHOST_KIND, strlen(VHOST_KIND)) == 0)) {
                vr_dpdk_handle_vhost0_notification
                                 (mtu, (ifi->ifi_flags & IFF_RUNNING));
            }
        }
    }
}
