...
walk_cb(struct rte_mempool *mp, void *arg __rte_unused) 
{
    ...
    /* redefining VR_INFO_FAILED to prevent VI_PRINTF to return non-void value */
    #undef VR_INFO_FAILED
    #define VR_INFO_FAILED

    /* redefining VR_INFO_MSG_TRUNC to prevent VI_PRINTF to return non-void value */
...
static int
display_xstats(VR_INFO_ARGS, uint16_t port_id, int xstats_count, char xstats_arg[]) 
{

    VR_INFO_DEC();
    struct rte_eth_xstat_name *xstats_names;
    uint64_t *values;
    int ret, i, segment_lookup[6][xstats_count], rpi = 0, rbi = 0, tpi = 0, tbi = 0, ei = 0, oth = 0;
    char seperator[SEPERATOR];
    bool is_all = !strcmp(xstats_arg, "all");
    memset(seperator, '-', SEPERATOR);
    seperator[SEPERATOR] = '\0';
    ...
    for (i = 0; i < rpi - 1; i++) {
        if (values[segment_lookup[RX_PACKETS][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[RX_PACKETS][i]].name, values[segment_lookup[RX_PACKETS][i]]);
        }
    ...
    for (i = 0; i < tpi - 1; i++) {
        if (values[segment_lookup[TX_PACKETS][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[TX_PACKETS][i]].name, values[segment_lookup[TX_PACKETS][i]]);
        }
    ...
    VI_PRINTF("Rx Bytes: \n");
    for (i = 0; i < rbi - 1; i++) {
        if (values[segment_lookup[RX_BYTES][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[RX_BYTES][i]].name, values[segment_lookup[RX_BYTES][i]]);
        }
    ...
    VI_PRINTF("Tx Bytes: \n");
    for (i = 0; i < tbi - 1; i++) {
        if (values[segment_lookup[TX_BYTES][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[TX_BYTES][i]].name, values[segment_lookup[TX_BYTES][i]]);
        }
    ...
    VI_PRINTF("Errors: \n");
    for (i = 0; i < ei - 1; i++) {
        if (values[segment_lookup[ERRORS][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[ERRORS][i]].name, values[segment_lookup[ERRORS][i]]);
        }
    ...
    VI_PRINTF("Others: \n");
    for (i = 0; i < oth - 1; i++) {
        if (values[segment_lookup[OTHERS][i]] != 0) {
            VI_PRINTF("\t%s: %"
                PRIu64 "\n", xstats_names[segment_lookup[OTHERS][i]].name, values[segment_lookup[OTHERS][i]]);
        }
...
dpdk_info_get_xstats(VR_INFO_ARGS) 
{
    ...
    if (port_id != VR_DPDK_INVALID_PORT_ID) {
        ...
        if (!strcmp(msg_req->inbuf, "") || !strcmp(msg_req->inbuf, "all")) {
            reqd_interface = 0;
        } else if (!strcmp(msg_req->inbuf, "0") || (atoi(msg_req->inbuf) > 0 && atoi(msg_req->inbuf) < 7)) {
            reqd_interface = 1;
        } else {
            RTE_LOG(ERR, VROUTER, "Invalid argument.\n");
            return -1;
        ...
        case 0:
            ...
            display_xstats(msg_req, port_id, xstats_count, msg_req->inbuf);
            ...
            for (i = 0; i < ethdev->ethdev_nb_slaves; i++) {
                ...
                display_xstats(msg_req, slave_id, xstats_count, msg_req->inbuf);
            }
            break;
        case 1:
            if (!strcmp(msg_req->inbuf, "0")) {
                ...
                if (atoi(msg_req->inbuf) <= ethdev->ethdev_nb_slaves) {
