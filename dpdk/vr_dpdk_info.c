...
display_xstats(VR_INFO_ARGS, uint16_t port_id, int xstats_count, bool is_all)
{
    ...
    values = (uint64_t *)vr_zalloc(sizeof( *values) * xstats_count, VR_INFO_REQ_OBJECT);
    if (values == NULL) {
        RTE_LOG(ERR, VROUTER, "Cannot allocate memory for xstats\n");
        return -1;
    }

    xstats_names = (struct rte_eth_xstat_name *)vr_zalloc(sizeof(struct rte_eth_xstat_name) * xstats_count, 
                       VR_INFO_REQ_OBJECT);
...
dpdk_info_get_xstats(VR_INFO_ARGS)
{
    ...
    if (port_id != VR_DPDK_INVALID_PORT_ID) {
        ...
        if (!strcmp(msg_req->inbuf, "") || is_all) {
            ...
            (atoi(msg_req->inbuf) > 0 && atoi(msg_req->inbuf) <= VR_DPDK_BOND_MAX_SLAVES)) {
            reqd_interface = 1;
        } else {
            RTE_LOG(ERR, VROUTER, "Invalid argument.\n");
            return -1;
