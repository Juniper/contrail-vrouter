...
/*
 * vr_uvhost_client_stop - stop UVHost client and free all the resources.
 */
void
vr_uvhost_client_force_stop(vr_uvh_client_t *vru_cl)
{
    int i;
    ...
    for (i = 0; i < VR_DPDK_VIRTIO_MAX_QUEUES*2; i++) {
        /* Disable all the virtio queues. */
        vr_dpdk_virtio_ready_set(vru_cl->vruc_idx, i, VQ_NOT_READY);
    }
...
/*
 * vr_uvhost_client_check_stop - check if client is ready to stop.
 *
 * Returns 0 if the client is stopped, -1 otherwise.
 */
int
vr_uvhost_client_check_stop(vr_uvh_client_t *vru_cl)
{
    int i;
    ...
    for (i = 0; i < VR_DPDK_VIRTIO_MAX_QUEUES*2; i++) {
        if (vr_dpdk_virtio_is_ready(vif_idx, i))
            return -1;
    }

    /* Now client is ready to stop. */
    vr_uvhost_client_force_stop(vru_cl);

    return 0;
}

/*
 * vr_uvhost_client_start - start UVHost client.
 */
static void
uvhost_client_start(vr_uvh_client_t *vru_cl)
{
    int i;
    ...
    /* Set all the virtio queues to a ready state. */
    for (i = 0; i < VR_DPDK_VIRTIO_MAX_QUEUES*2; i++) {
        vr_dpdk_virtio_ready_set(vru_cl->vruc_idx, i, VQ_READY);
    }
    vru_cl->vruc_flags |= VR_UVH_CLIENT_STARTED;
...
/*
 * vr_uvhost_client_check_start - check if client is ready to use and starts
 * the client.
 *
 * Returns 0 if the client is started, -1 otherwise.
 */
int
vr_uvhost_client_check_start(vr_uvh_client_t *vru_cl)
{
    int i;
    unsigned int vif_idx = vru_cl->vruc_idx;
    ...
    /* Check if all the virtio queues are ready to start. */
    for (i = 0; i < VR_DPDK_VIRTIO_MAX_QUEUES*2; i++) {
        if (!vr_dpdk_virtio_can_start(vif_idx, i))
            return -1;
    }

    /* Now client is ready for forwarding. */
    uvhost_client_start(vru_cl);
