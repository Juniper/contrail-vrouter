...
unsigned int vr_dpdk_master_port;

/* On few platforms, Master lsc(callback) is not called.
 * Forcing here to send Master notification is sent before
 * sending child interface */
static bool vr_dpdk_master_init = 0;
        ...
        void *param, void *ret_param __rte_unused)
{
    ...
    if(vr_dpdk_master_port != port_id) {
        if(!vr_dpdk_master_init) {
            vr_dpdk_bond_send_port_info(vr_dpdk_master_port, vif_idx);
            vr_dpdk_master_init = 1;
        }
    }
    else {
        vr_dpdk_master_init = 1;
        vr_printf("Master initialized\n");
    }
