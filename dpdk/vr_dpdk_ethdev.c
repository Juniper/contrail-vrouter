                                          ...
                                          struct vr_ip6 *ipv6_hdr)
{
    ...
    /**
     * At this point ip_addr_ptr points either to the beggining of IPv4 source
     * address or to the middle of IPv6 destination address. In both cases it is
     * 8 bytes before the beggining of the L4 header, assuming neither options
     * in IPv4 header, nor extended IPv6 headers are present. This allows us to
     * reach L4 header by moving ip_addr_ptr by 8 bytes.
     */
    switch (ip_proto) {
...
#endif
        l4_ptr = (uint32_t *)((uintptr_t)ip_addr_ptr + 8);
...
dpdk_mbuf_parse_and_hash_packets(struct rte_mbuf *mbuf)
{
    ...
    if (eth_hdr->eth_proto == rte_cpu_to_be_16(VR_ETH_PROTO_VLAN)) {
        if (unlikely(mbuf_data_len < pull_len + VR_VLAN_HLEN))
            return -1;

        eth_hdr = (struct vr_eth *)((uintptr_t)eth_hdr + VR_VLAN_HLEN);
    }
    ...
    if (likely(eth_hdr->eth_proto == rte_cpu_to_be_16(VR_ETH_PROTO_IP))) {
        ...
        ipv6_hdr = (struct vr_ip6 *)((uintptr_t)eth_hdr + VR_ETHER_HLEN);
