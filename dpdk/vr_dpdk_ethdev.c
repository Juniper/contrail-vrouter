 ...
 */
static void
vr_dpdk_nl_send_bond_intf_state(int state, struct vr_dpdk_bond_member_info *info,
        unsigned type)
{
    int nl_fd;
    char buf[1024];
    ...
    len = NLA_HDRLEN + NLA_ALIGN(sizeof(*info) + 1);

    nla->nla_len = len;
    nla->nla_type = IFLA_IFNAME;

    if_name_buf = (char *)nla + NLA_HDRLEN;
    memcpy(if_name_buf, info, sizeof(*info));
    ...
    RTE_LOG(INFO, VROUTER, "Notifed Link status update to agent for interface name %s\n",
            info->intf_name);
        ...
        void *param, void *ret_param __rte_unused)
{
    ...
    if(rte_eth_devices[port_id].device->driver->name != NULL)
        strcpy(member_info.intf_drv_name, rte_eth_devices[port_id].device->driver->name);
    
    RTE_LOG(INFO, VROUTER, "Port ID: %d Link Status: %s intf_name:%s drv_name:%s \n\n",
            port_id, (link.link_status?str[0]:str[1]), member_info.intf_name,
            member_info.intf_drv_name);

    dev_flags = rte_eth_devices[port_id].data->dev_flags;
    bond_type = (dev_flags & RTE_ETH_DEV_BONDED_SLAVE) ? VR_DPDK_BOND_SLAVES : VR_DPDK_FABRIC;

    vr_dpdk_nl_send_bond_intf_state(link.link_status, &member_info, bond_type);

    return 0;
}

static void
vr_dpdk_notif_agent_bond_init(uint8_t port)
{
    int ret = 0, dev_flags;
    uint8_t link = 0, bond_type;

    struct vr_dpdk_bond_member_info member_info;
    
    ret = rte_eth_dev_get_name_by_port(port, member_info.intf_name);
    if(ret != 0)
        RTE_LOG(ERR, VROUTER, "Error getting bond interface name\n");
    
    if(rte_eth_devices[port].device->driver->name != NULL)
        strcpy(member_info.intf_drv_name, rte_eth_devices[port].device->driver->name);
    
    dev_flags = rte_eth_devices[port].data->dev_flags;
    bond_type = (dev_flags & RTE_ETH_DEV_BONDED_SLAVE) ? VR_DPDK_BOND_SLAVES : VR_DPDK_FABRIC;

    /* Notifying agent about the Fabric(Master)/Slave interface members 
     * Default link status is DOWN */
    vr_dpdk_nl_send_bond_intf_state(link, &member_info, bond_type);
}
...
vr_dpdk_bond_intf_cb_register(struct vr_dpdk_ethdev *ethdev)
{
    ...
    if(ret)
        ...
        vr_dpdk_notif_agent_bond_init(port_id);
    ...
    for (i = 0; i < ethdev->ethdev_nb_slaves; i++) {
        ...
        ret = rte_eth_dev_callback_register(port_id, RTE_ETH_EVENT_INTR_LSC,
                vr_dpdk_bond_intf_callback, NULL);
        if(ret)
            RTE_LOG(ERR, VROUTER, "Failed to setup callback for event RTE_ETH_EVENT_INTR_LSC \
                    for portid: %d\n", port_id);
        else
            vr_dpdk_notif_agent_bond_init(port_id);
    }
