        ...
        void *param, void *ret_param __rte_unused)
{
    ...
    bond_type = (dev_flags & RTE_ETH_DEV_BONDED_SLAVE) ? VR_DPDK_BOND_SLAVE : VR_DPDK_FABRIC;

    vr_dpdk_nl_send_bond_intf_state(link.link_status, &member_info, bond_type, vif_idx);

    return 0;
}

static void
vr_dpdk_notif_agent_bond_init(uint8_t port, uint8_t vif_idx)
{
    int ret = 0, dev_flags;
    ...
    vr_dpdk_nl_send_bond_intf_state(link, &member_info, bond_type, vif_idx);
}
...
vr_dpdk_bond_intf_cb_register(struct vr_dpdk_ethdev *ethdev)
{
    ...
    if(ret)
        ...
        vr_dpdk_notif_agent_bond_init(port_id, ethdev->ethdev_vif_idx);
