...
dpdk_lcore_rx_ring_vroute(struct vr_dpdk_lcore *lcore, struct rte_ring *ring)
{
    ...
    if (likely(ret == 0)) {
        ...
        if (likely(vif != NULL) && vif->vif_gen == vif_gen) {
            ...
            for (i = 1; i < nb_pkts; i++)
                vr_dpdk_pfree(pkts[i], vif, VP_DROP_INTERFACE_DROP);
        }
    }
        ...
        if (likely(nb_pkts != 0)) {
            ...
            if (likely(rtp->rtp_tx_queue->txq_ops.f_tx != NULL)) {
                ...
                for (i = 0; i < nb_pkts; i++)
                    ...
                    vr_dpdk_pfree(pkts[i], rtp->rtp_tx_queue->q_vif, VP_DROP_INTERFACE_DROP);
            }
        }
        rtp++;
    }
...
dpdk_lcore_vlan_fwd(struct vr_dpdk_lcore* lcore)
{
    ...
    for (; i < nb_pkts; i++) {
        struct vr_packet *pkt = vr_dpdk_mbuf_to_pkt(pkts[i]);
        vr_dpdk_pfree(pkts[i], pkt->vp_if, VP_DROP_VLAN_FWD_TX);
    }
}
