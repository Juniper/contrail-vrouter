...
static void
vr_dpdk_get_queue_to_lcore(struct vr_interface *vif, int *queue_to_lcore)
{
    ...
    lcore_id = VR_DPDK_PACKET_LCORE_ID;
    do {
        lcore = vr_dpdk.lcores[lcore_id];
        queue = lcore->lcore_hw_queue[vif->vif_idx];
        if (queue >= 0) {
            queue_to_lcore[queue] = lcore_id;
        }
        ...
        uint16_t nb_tx_queues, vr_dpdk_queue_init_op tx_queue_init_op)
{
    ...
    if (nb_tx_queues > VR_DPDK_MAX_NB_TX_QUEUES) {
        if (num_fwd_cores > VR_DPDK_MAX_NB_TX_QUEUES) {
            return -EINVAL;
        }
    }
    ...
    do {
        ...
        if (lcore_id >= VR_DPDK_PACKET_LCORE_ID) {
            if (vif->vif_num_hw_queues) {
                if (queue_index >= vif->vif_num_hw_queues)
                    break;
    ...
    } while ((lcore_id = rte_get_next_lcore(lcore_id, 1, 1)) != least_used_id);
...
{
    int queue_to_lcore[VR_DPDK_MAX_NB_TX_QUEUES] = { -1 };
    ...
    vr_dpdk_get_queue_to_lcore(vif, queue_to_lcore);

    /* init TX queues starting with the least used lcore */
    lcore_id = least_used_id;
    /* for all lcores */
    do {
        if (!vif->vif_num_hw_queues && (num_queues_per_lcore == 1)) {
            break;
        }
        ...
        if (lcore_id >= VR_DPDK_PACKET_LCORE_ID) {
            ...
            for (queue_index = 0; queue_index < num_queues_per_lcore;
                ...
                if (vif->vif_hw_queues) {
                    queue = vif->vif_hw_queues[queue_index];
                    host_lcore_id = queue_to_lcore[queue];
                } else {
                    queue = queue_index;
                    host_lcore_id = least_used_id;
                }

                if (lcore->lcore_hw_queue[vif->vif_idx] != queue) {
...
dpdk_lcore_rxtx_release_all(struct vr_interface *vif)
{
    ...
    RTE_LCORE_FOREACH_SLAVE(lcore_id) {
        ...
        for (i = 0; i < lcore->num_tx_queues_per_lcore[vif->vif_idx]; i++) {
            if (lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
                queue = lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx][i];
            } else {
                queue = i;
            }
    ...
    if (eth_vif) {
        ...
        if (tx_queue->txq_ops.f_tx) {
            if (vr_dpdk.kni_state > 0)
                ...
                nb_pkts = vr_dpdk_tapdev_dequeue_burst(vr_dpdk.vlan_dev, pkts,
                        VR_DPDK_RX_BURST_SZ);
            for (i = 0; i < nb_pkts; i++)
