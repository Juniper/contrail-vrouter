...
dpdk_vif_queue_setup(struct vr_interface *vif)
{
    ...
    do {
		...
		if ((lcore >= VR_DPDK_PACKET_LCORE_ID) && vif_max_queue > 0) {
...
dpdk_if_tx(struct vr_interface *vif, struct vr_packet *pkt)
{
    ...
    RTE_LOG(DEBUG, VROUTER,"%s: TX packet to interface %s\n", __func__,
        vif->vif_name);
    RTE_LOG(DEBUG, VROUTER, "queue %d in lcore %u (hw_queue %u)\n",
            pkt->vp_queue, lcore_id, lcore->lcore_hw_queue[vif_idx]);
...
dpdk_dev_stats_update(struct vr_interface *vif, unsigned lcore_id)
{
    ...
    queue_id = lcore->lcore_hw_queue[vif->vif_idx];
    if (queue_id < 0) {
        queue_id = 0;
    }

    if (lcore->lcore_tx_queues[vif->vif_idx]) {
        queue = &lcore->lcore_tx_queues[vif->vif_idx][queue_id];
