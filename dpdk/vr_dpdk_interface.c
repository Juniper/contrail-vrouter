...
static uint16_t
dpdk_gro_rx_csum_fixup(struct gro_entry *entry, struct vr_ip *ip4, struct vr_tcp *tcph,
...
static int 
dpdk_gro_flush(struct gro_ctrl *gro, struct vr_dpdk_gro_flow_key *key, struct gro_entry *entry)
{
    int ret;
    struct vr_tcp *tcph;
    ...
    if (entry->mbuf_cnt > 1) {
        ...
        entry->ulp_csum += entry->p_len - sizeof(*ip4);
    ...
    ret = rte_hash_del_key(gro->gro_tbl_handle, key);
    ASSERT(ret >= 0);
...
{
	uint32_t iter = 0, cnt=0;
    struct vr_dpdk_gro_flow_key *next_key;
    ...
    if (gro->gro_tbl_handle == NULL)
        return;

    cur_cycles = rte_get_timer_cycles();

	/* Iterate through the hash table */
	while (rte_hash_iterate(gro->gro_tbl_handle, (const void**)&next_key, 
                                                             (void**)&entry, &iter) >= 0) {
...
{
    struct vr_dpdk_gro_flow_key flow;
    struct gro_entry *entry;
    struct rte_mbuf *m = vr_dpdk_pkt_to_mbuf(pkt);
    ...
    if (vr_ip_is_ip6(nw_hdr)) {
        ...
        flow.ip_src = ip4->ip_saddr; 
        flow.ip_dst = ip4->ip_daddr;
        tcph = (struct vr_tcp *)(nw_hdr + hlen);
        csum = rte_be_to_cpu_16(tcph->tcp_csum); 
        flow.port_src = rte_be_to_cpu_16(tcph->tcp_sport);
        flow.port_dst = rte_be_to_cpu_16(tcph->tcp_dport);
        flow.vif_idx = vif->vif_idx;
    ...
    ret = rte_hash_lookup_data(lcore->gro.gro_tbl_handle, &flow, (void*)&entry);
    ...
    if (ret >= 0) {
        ...
        if (entry->p_len > (65535 - tcp_data_len)) {
            dpdk_gro_flush(&lcore->gro, &flow, entry);
            goto create;
        }
        ...
        entry->mbuf_cnt++;
        entry->ulp_csum += dpdk_gro_rx_csum_fixup(entry, ip4, tcph, tcp_data_len, ~csum);
...
create:
        ...
        lcore->gro.gro_flows++;
        if(rte_hash_add_key_data(lcore->gro.gro_tbl_handle, &flow, entry) < 0)
            ret = GRO_ERROR;
        else
            ret = GRO_MERGED;
    }
