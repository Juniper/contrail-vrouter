...
static int
dpdk_if_get_slaves_info(struct vr_interface *bond_vif, unsigned core, struct vr_bond_slaves_info *bond_slave_info)

{
    ...
    bond_slave_info->nbr_bond_slaves = rte_eth_bond_slaves_get(bond_vif->vif_idx, bond_slaves, sizeof(bond_slaves));
    ...
    for (slave_index = 0; slave_index < bond_slave_info->nbr_bond_slaves; slave_index++) {
        ...
        memcpy(bond_slave_info->bond_slaves[slave_index].mac_addr, mac_addr.addr_bytes, ETHER_ADDR_LEN);
    }
    return ((bond_slave_info->nbr_bond_slaves == -1)?-EFAULT:0);
}
