...
dpdk_af_packet_if_add(struct vr_interface *vif) {
    ...
    RTE_LOG(INFO, VROUTER, "Adding AF_PACKET interface %s type %d transport %d\n",
            vif->vif_name, vif->vif_type, vif->vif_transport);

    ret = snprintf(name, VR_INTERFACE_NAME_LEN, "eth_af_packet_%d", vif->vif_idx);
    if (ret < 0) {
        RTE_LOG(ERR, VROUTER, "Error creating name for AF_PACKET");
        return ret;
    }
    ...
    if (ret < 0) {
        RTE_LOG(ERR, VROUTER, "Error creating config for AF_PACKET\n");
        return ret;
    }

    /* DPDK af_packet driver hack: rte_pmd_af_packet_devinit returns PMD ID */
    port_id = rte_pmd_af_packet_devinit(name, params);
    if (port_id < 0) {
        RTE_LOG(ERR, VROUTER, "Error initializing AF_PACKET device %"PRIu8"\n",
                port_id);
        return port_id;
    }
