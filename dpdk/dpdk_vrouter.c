...
char core_mask_str[VR_DPDK_STR_BUF_SZ];
char thread_mask_str[VR_DPDK_STR_BUF_SZ];
char *service_core_mask_str = NULL;
char *dpdk_ctrl_thread_mask_str = NULL;
...
char *
vr_core_mask_util(unsigned int core_mask, char str[])
{
    int i = 0, j = 0;
    str[j++] = '(';
    while (core_mask != 0) {
...
dpdk_argv_update(void)
{
    ...
    if (vr_service_core_mask != 0) {
        service_core_mask_str = vr_core_mask_util(vr_service_core_mask, core_mask_str);
    }
    else {
        if (!service_core_mask_str) {
            sprintf(core_mask_str, "(0-%ld)", (system_cpus_count - 1));
            service_core_mask_str = core_mask_str;
	}
    }

    if (vr_dpdk_ctrl_thread_mask != 0) {
        dpdk_ctrl_thread_mask_str = vr_core_mask_util(vr_dpdk_ctrl_thread_mask, thread_mask_str);
    }
...
Usage(void)
{
	...
	"    --"PKT_DROP_LOG_BUFFER_SIZE_OPT" NUM Maximum debug log buffer size\n"
        ...
        "    --"VR_SERVICE_CORE_MASK_OPT" NUM LIST OR HEXADECIMAL BITMASK Configurable parameter for service core mask\n"
	"    --"VR_DPDK_CTRL_THREAD_MASK_OPT" NUM LIST OR HEXADECIMAL BITMASK  Configurable parameter for dpdk control threads\n"
        );
