...
#define VR_LOG_MOD_SIZES_OPT "sizes"
    VR_LOG_MOD_SIZES_OPT_INDEX,
    MAX_OPT_INDEX
};
...
static struct option long_options[] = {
    ...
    [VR_LOG_ENABLE_OPT_INDEX]       =   {VR_LOG_ENABLE_OPT, required_argument,
						    NULL,                   0},
    [VR_LOG_MOD_SIZES_OPT_INDEX]    =   {VR_LOG_MOD_SIZES_OPT, required_argument,
						    NULL,                   0},
...
Usage(void)
{
	...
	"    --"VR_LOG_MOD_SIZES_OPT" Module log sizes\n"
        );
...
parse_long_opts(int opt_flow_index, char *optarg)
{
	...
	break;
    case VR_LOG_MOD_SIZES_OPT_INDEX:
	sizes[i] = (unsigned int)strtoul(optarg, NULL, 0);
	if(errno != 0) {
		sizes[i] = VR_LOG_MAX;
	}
        i++;
