...
char core_mask_str_util[VR_DPDK_STR_BUF_SZ];
char *service_core_mask_str;
char *dpdk_ctrl_thread_mask_str;
...
char *
vr_core_mask_util(unsigned int core_mask)
{
    char *core_mask_str = (char *) malloc(VR_DPDK_STR_BUF_SZ);
    int i = 0, j = 0;
    core_mask_str[j++] = '(';
    while (core_mask != 0) {
	if ((core_mask & 1) == 1) {
...
dpdk_argv_update(void)
{
    ...
    if (vr_service_core_mask == 0) {
        if (!service_core_mask_str) {
            sprintf(core_mask_str_util, "(0-%ld)", (system_cpus_count - 1));
	    service_core_mask_str = core_mask_str_util;
        }
    }
    else {
        service_core_mask_str = vr_core_mask_util(vr_service_core_mask);
    }

    if (vr_dpdk_ctrl_thread_mask != 0) {
        dpdk_ctrl_thread_mask_str = vr_core_mask_util(vr_dpdk_ctrl_thread_mask);
    }
    if (dpdk_ctrl_thread_mask_str) {
        if (dpdk_argv_append("-t", dpdk_ctrl_thread_mask_str) != 0)
	        return -1;
    }
    ...
    RTE_LOG(INFO, VROUTER, "VR_DPDK_CTRL_THREAD_MASK:    %x\n",
...
Usage(void)
{
	...
	"    --"VR_DPDK_CTRL_THREAD_MASK_OPT" NUM Configurable parameter for dpdk control threads\n"
        );
...
parse_long_opts(int opt_flow_index, char *optarg)
{
    ...
    case VR_SERVICE_CORE_MASK_OPT_INDEX:
        vr_service_core_mask = (int) strtoul(optarg, NULL, 0);
        if (vr_service_core_mask == 0) {
           service_core_mask_str = optarg;
        }
        break;

    case VR_DPDK_CTRL_THREAD_MASK_OPT_INDEX:
	vr_dpdk_ctrl_thread_mask = (int) strtoul(optarg, NULL, 0);
	if (vr_dpdk_ctrl_thread_mask == 0) {
            dpdk_ctrl_thread_mask_str = optarg;
	}
	break;
