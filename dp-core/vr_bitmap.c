...
vr_bitmap_valid_bit(vr_bmap_t b, unsigned int bit)
{
    ...
    data = bmap->bmap_data[(bit / VR_BITMAP_STRIDE_LEN)];
    if (data && (1 << (bit % VR_BITMAP_STRIDE_LEN)))
        return true;
    ...
    for (i = 0; i < bmap->bmap_size; i++, stride_num++) {
        ...
        for (j = 0; ((bmap->bmap_data[stride_num] != 0xFFFFFFFF) &&
            ...
            if (__sync_bool_compare_and_swap(&bmap->bmap_data[stride_num],
                ...
                __sync_add_and_fetch(&bmap->bmap_used_bits, 1);
...
vr_bitmap_clear_bit(vr_bmap_t b, unsigned int bit)
{
    ...
    __sync_and_and_fetch(&bmap->bmap_data[(bit / VR_BITMAP_STRIDE_LEN)],
                            (~(1 << (bit % VR_BITMAP_STRIDE_LEN))));
    __sync_sub_and_fetch(&bmap->bmap_used_bits, 1);
