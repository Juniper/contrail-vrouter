                                    ...
                                    vr_hentry_t **prev_entry)
{
    ...
    vr_hentry_t *head_ent, *prev = NULL;
    struct vr_htable *table = (struct vr_htable *)htable;
...
vr_hentry_defer_delete(void *arg)
{
    ...
    vr_hentry_t *entry, *prev_entry = NULL;
    ...
    if (entry) {
        ...
        prev_entry = vr_get_hentry_by_index(delete_data->hd_htable,
                                        delete_data->hd_prev_index);

        if (__vr_release_hentry(delete_data->hd_htable, entry, &prev_entry)) {
            delete_data->hd_prev_index = prev_entry->hentry_index;
...
vr_release_hentry(vr_htable_t htable, vr_hentry_t *entry)
{
    unsigned int index = entry->hentry_index;
    struct vr_hentry_delete_data *delete_data;
    vr_hentry_t *prev_entry = NULL;

retry:
    if (__vr_release_hentry(htable, entry, &prev_entry)) {
        ...
        delete_data->hd_index = prev_entry->hentry_index;
