...
vr_hentry_scheduled_delete(void *arg)
{
    ...
    /*
     * The list MUST be traversed as many times as it has been
     * scheduled. This is to ensure that, if schudule happens for an
     * entry which has already been traversed, it should not be missing
     * the deletion. There is a chance that, some traversals might not
     * process anything
     */
    ...
    } while (__sync_sub_and_fetch(&delete_data->hd_scheduled, 1));
...
{
    unsigned int cpu_num, delete_index, index = ent->hentry_index;
    ...
    ent = vr_get_hentry_by_index(htable, ent->hentry_bucket_index);
    delete_index = ent->hentry_index / table->ht_bucket_size;
    delete_data = vr_btable_get(table->ht_dtable, delete_index);

    /* Schdule the deltion only if it is not already scheduled */
    if (__sync_add_and_fetch(&delete_data->hd_scheduled, 1) == 1) {

        delete_data->hd_table = (struct vr_htable *)htable;
        delete_data->hd_index = ent->hentry_index;

        /* Schedule the deletion on a cpu based on bucket index */
        cpu_num = ent->hentry_index % vr_num_cpus;
