...
__vr_release_hentry(vr_htable_t htable, vr_hentry_t *entry,
                                            vr_hentry_t **prev_entry)
{
    unsigned int index = entry->hentry_index;
    vr_hentry_t *head_ent, *prev = NULL, *pprev;
    struct vr_htable *table = (struct vr_htable *)htable;
    ...
    /*
     * IF the entry is overflow entry, it is not unlinked from list if
     * the previous entry is also under deletion. In that case, we defer
     * the deletion. While defering, we keep previous entry to previous
     * entry in defer data, so that, while processing defer data, we
     * have previous entry pointer, without looping again. We keep
     * previous entry to previous, because previous is expected to be
     * delted
     */
    if (index >= table->ht_hentries) {


        /* Mark the entry saying it is under deletion */
        entry->delete_marked = true;
        ...
        if (prev_entry && (*prev_entry)) {
            ...
            if (prev->hentry_next != entry) {
                ...
                if (prev && prev->hentry_next != entry)
                    prev = NULL;
            }
        }
        ...
        if (((prev == head_ent) || (!prev->delete_marked))) {
...
vr_hentry_defer_delete(void *arg)
{
    ...
    if (entry) {
        ...
        prev = vr_get_hentry_by_index(delete_data->hd_htable,
                delete_data->hd_prev_index);

        if (__vr_release_hentry(delete_data->hd_htable, entry, &prev)) {
            delete_data->hd_prev_index = prev->hentry_index;
...
vr_release_hentry(vr_htable_t htable, vr_hentry_t *entry)
{
    unsigned int index = entry->hentry_index;
    struct vr_hentry_delete_data *delete_data;
    struct vr_hentry *prev = NULL;

retry:
    if (__vr_release_hentry(htable, entry, &prev)) {
