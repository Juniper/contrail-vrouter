...
vr_fragment_queue_free(struct vr_fragment_queue *queue)
{
    ...
    while (vfqe) {
        ...
        if (vfqe->fqe_pnode.pl_packet)
            DS_LOG(VP_DROP_MISC, 0, 0, VR_FRAGMENT_C, __LINE__);
        ...
        struct vr_fragment_queue_element *vfqe)
{
    ...
    if (!found) {
        if (frag_head) {
            ...
            DS_LOG(drop_reason, pkt, 0, VR_FRAGMENT_C, __LINE__);
            goto exit_assembly;
        }
    ...
    if (!frag_head) {
        ...
        DS_LOG(VP_DROP_CLONED_ORIGINAL, pkt, 0, VR_FRAGMENT_C, __LINE__);
        vr_fragment_queue_element_free(vfqe, VP_DROP_CLONED_ORIGINAL);
    }
...
fail:
    ...
    DS_LOG(VP_DROP_FRAGMENTS, pkt, 0, VR_FRAGMENT_C, __LINE__);
