        ...
        struct vr_forwarding_md *fmd)
{
    if (vif_is_virtual(pkt->vp_if)) {
        ...
        struct vr_interface *hbf_r = vr_get_hbf_right_vif(router);
        if (hbf_r) {
            struct vr_eth *eth = (struct vr_eth*)pkt_data(pkt);
            uint32_t *index_ptr = (uint32_t*)(eth->eth_dmac + 2);
            uint16_t *magic_ptr = (uint16_t*)(eth->eth_dmac);
            *index_ptr = htonl(index);
            *magic_ptr = htons(0xC0DE);
            pkt->vp_if = hbf_r;
            hbf_r->vif_tx(hbf_r, pkt, fmd);
                ...
                struct vr_forwarding_md *fmd)
{
    ...
    } else if (vr_is_hbf_left_vif(pkt->vp_if->vif_name)) {
