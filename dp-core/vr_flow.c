...
{
    struct vr_flow_entry *fe = (struct vr_flow_entry *)entry;
...
vr_htable_t
vr_flow_table_attach(void *flow_table, void *oflow_table)
{
    return vr_htable_attach(vrouter_get(0), vr_flow_entries, flow_table,
            vr_oflow_entries, oflow_table, sizeof(struct vr_flow_entry),
            0, 0, vr_get_flow_key);
}
...
vr_flow_table_init(struct vrouter *router)
{
    if (!router->vr_flow_table) {
        ...
        router->vr_flow_table = vr_htable_create(router, vr_flow_entries,
                vr_oflow_entries, sizeof(struct vr_flow_entry), 0, 0,
                vr_get_flow_key);
