        ...
        struct vr_flow_stats *stats, struct vr_forwarding_md *fmd)
{
    ...
    if (!vfq) {
        ...
        DS_LOG(drop_reason, pkt, 0, VR_FLOW_C, __LINE__);
        goto drop;
    }
        ...
        struct vr_packet *pkt, struct vr_forwarding_md *fmd)
{
    ...
    DS_LOG(VP_DROP_FLOW_ACTION_INVALID, pkt, 0, VR_FLOW_C, __LINE__);
        ...
        struct vr_forwarding_md *fmd)
{
    ...
    if (src_nh->nh_validate_src) {
        ...
        if (valid_src == NH_SOURCE_INVALID) {
            DS_LOG(VP_DROP_INVALID_SOURCE, pkt, 0, VR_FLOW_C, __LINE__);
    ...
    case VR_FLOW_ACTION_DROP:
        DS_LOG(VP_DROP_FLOW_ACTION_DROP, pkt, 0, VR_FLOW_C, __LINE__);
                           ...
                           unsigned short *drop_reason)
{
    ...
    if (vif_l && vif_drop_new_flows(vif_l)) {
        DS_LOG(VP_DROP_NEW_FLOWS, pkt, 0, VR_FLOW_C, __LINE__);
    ...
    *drop_reason = VP_DROP_FLOW_UNUSABLE;
    ...
    if (pkt->vp_type == VP_TYPE_IP) {
        if (!vr_inet_flow_allow_new_flow(router, pkt)) {
            DS_LOG(VP_DROP_FLOW_UNUSABLE, pkt, 0, VR_FLOW_C, __LINE__);
            *drop_reason = VP_DROP_FLOW_UNUSABLE;
            return false;
        }
    }
               ...
               struct vr_packet *pkt, struct vr_forwarding_md *fmd)
{
    ...
    if (!flow_e) {
        ...
        if (!vr_flow_allow_new_flow(router, pkt, &drop_reason, &burst)) {
            DS_LOG(drop_reason, pkt, key , VR_FLOW_C, __LINE__);
            vr_pfree(pkt, drop_reason);
            return FLOW_CONSUMED;
        }
        ...
        flow_e = vr_flow_get_free_entry(router, key, pkt->vp_type,
                true, &fe_index);
        if (!flow_e) {
            DS_LOG(VP_DROP_FLOW_TABLE_FULL, pkt, key, VR_FLOW_C, __LINE__);
        ...
        struct vr_forwarding_md *fmd)
{
    ...
    case FLOW_EVICT_DROP:
        DS_LOG(VP_DROP_FLOW_EVICT, pkt, 0, VR_FLOW_C, __LINE__);
        ...
        struct vr_flow_entry *fe, struct vr_forwarding_md *fmd)
{
    ...
    if (!vif || (pkt->vp_if != vif)) {
        pkt->vp_if = NULL;
        DS_LOG(VP_DROP_INVALID_IF, pkt, 0, VR_FLOW_C, __LINE__);
