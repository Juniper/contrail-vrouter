    ...
    if (fe) {
        ...
        if (!(flags & VR_FLOW_FLAG_ACTIVE)) {
            ...
            fe->fe_flags = ((flags & ~VR_FLOW_FLAG_EVICTED) |
                     VR_FLOW_FLAG_NEW_FLOW);
            vr_printf("New for evicted flow %u (flags %x)\n",
                    fe->fe_hentry.hentry_index, fe->fe_flags);
        }
...
vr_flow_set(struct vrouter *router, vr_flow_req *req)
{
    ...
    if (fe->fe_flags & VR_FLOW_FLAG_NEW_FLOW) {
        req->fr_stats_bytes = fe->fe_stats.flow_bytes;
        req->fr_stats_packets = fe->fe_stats.flow_packets;
        req->fr_stats_oflows = ((fe->fe_stats.flow_packets_oflow << 16) |
                (fe->fe_stats.flow_bytes_oflow));
        memset(&fe->fe_stats, 0, sizeof(struct vr_flow_stats));
        fe->fe_flags ^= VR_FLOW_FLAG_NEW_FLOW;
    }
