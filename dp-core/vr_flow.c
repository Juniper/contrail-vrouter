...
vr_flow_defer_cb(struct vrouter *router, void *arg)
{
    ...
    if (vfdd->vfdd_delete &&
        ...
        vr_reset_flow_entry(router, fe, vfdd->vfdd_fe_index);
    } else {
        rfe = vr_get_flow_entry(router, fe->fe_rflow);
        vr_flow_evict_flow(router, fe, vfdd->vfdd_fe_index);
        if (rfe)
            vr_flow_evict_flow(router, rfe, fe->fe_rflow);
    }
...
static void
vr_flow_reset_evict(struct vr_flow_entry *fe)
{
    unsigned short flags;
...
vr_flow_defer(struct vr_flow_md *flmd, struct vr_flow_entry *fe)
{
    ...
    if (!vdd || !vdd->vdd_data) {
        vr_flow_reset_evict(fe);

        if (!(flmd->flmd_flags & VR_FLOW_FLAG_ACTIVE)) {
            vr_reset_flow_entry(flmd->flmd_router, fe, flmd->flmd_index);
        }
...
{
    bool alloced = false, swapped = false;
    unsigned short flags;
    ...
    for (i = 0; i < bucket_size; i++) {
        ...
        if (flow_e &&
            ...
            if (!memcmp(&flow_e->fe_key, key, key->flow_key_len)) {
                if (!(flow_e->fe_flags & VR_FLOW_FLAG_EVICTED)) {
    ...
    if (fe->fe_tcp_flags & VR_FLOW_TCP_DEAD) {
        if (fe->fe_rflow) {
            ...
            if (rfe && (rfe->fe_rflow == index) &&
                    (rfe->fe_tcp_flags & VR_FLOW_TCP_DEAD)) {
                flags = rfe->fe_flags;
                if (flags & VR_FLOW_FLAG_ACTIVE) {
                    (void)__sync_fetch_and_or(&rfe->fe_flags,
                            VR_FLOW_FLAG_EVICT_CANDIDATE);
                }
            }
        }

        (void)__sync_fetch_and_or(&fe->fe_flags, VR_FLOW_FLAG_EVICT_CANDIDATE);
        if (__vr_flow_schedule_transition(router, fe, index, fe->fe_flags) < 0) {
            /* worst case... */
            vr_flow_reset_evict(fe);
        }
    }
...
vr_flow_work(void *arg)
{
    ...
    if (((fe->fe_flags &
                (VR_FLOW_FLAG_EVICT_CANDIDATE | VR_FLOW_FLAG_EVICTED)) &&
            !(flmd->flmd_flags & VR_FLOW_FLAG_EVICT_CANDIDATE)) ||
            (flmd->flmd_flags & VR_FLOW_FLAG_EVICT_CANDIDATE)) {
        vr_init_forwarding_md(&fmd);
...
vr_flow_set(struct vrouter *router, vr_flow_req *req)
{
    ...
    if (fe) {
        if (fe->fe_flags &
                (VR_FLOW_FLAG_EVICT_CANDIDATE | VR_FLOW_FLAG_EVICTED |
                 VR_FLOW_FLAG_NEW_FLOW)) {
            vr_printf("vrouter(flow): Operation not allowed on %u when"
