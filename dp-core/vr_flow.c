        ...
        struct vr_forwarding_md *fmd)
{
    ...
    if (fe->fe_flags1 & VR_FLOW_FLAG1_HBS_LEFT) {
        ...
        if (hbs_l) {
            ...
            *index_ptr = htonl(index);
            ...
            if (vif_is_virtual(pkt->vp_if))
                ...
                *magic_ptr = htons(0xCAF0 + VR_HBS_FROM_FABRIC);
            pkt->vp_if = hbs_l;
            /*
        ...
        if (hbs_r) {
             ...
             */
            struct vr_nexthop* knh =
                       __vrouter_get_nexthop(router, fe->fe_key.flow_nh_id);
            struct vr_eth *eth;
            uint32_t *index_ptr;
            uint16_t *magic_ptr;

            if (knh->nh_family == AF_BRIDGE) {
                /* Do nothing */
            } else if (knh->nh_family == AF_INET) {
                /* Add ethernet header if it's a L3-NH */
            ...
            index_ptr = (uint32_t*)(eth->eth_dmac + 2);
            magic_ptr = (uint16_t*)(eth->eth_dmac);
            *index_ptr = htonl(index);

            /* Encode packet source in the header */
            if (vif_is_virtual(pkt->vp_if))
                *magic_ptr = htons(0xC0D0 + VR_HBS_FROM_VMI);
            else if (vif_is_fabric(pkt->vp_if))
