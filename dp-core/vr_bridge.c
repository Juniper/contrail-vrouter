        ...
        struct vr_forwarding_md *fmd)
{
    ...
    if (!nh) {
        ...
        trap = true;
        /*
         * We do not know whether the pkt is from Root or Leaf. We need
         * to lookup the Source Bmac and identify its Etree status. As a
         * short cut, lets mark it as root
         */
        nh_flags = NH_FLAG_ETREE_ROOT;
    } else {
        if (!(be->be_flags & VR_BE_MAC_MOVED_FLAG) && (nh->nh_validate_src)) {
            valid_src = nh->nh_validate_src(pkt, nh, fmd, &nh_flags);
