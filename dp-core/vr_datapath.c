...
int
vr_get_l3_hdr_offset_from_eth(struct vr_eth *eth, int pkt_len,
                               unsigned short *l3_proto)
{
    unsigned short pull_len, eth_proto;
    unsigned char *data = (unsigned char *)eth;
    ...
    eth_proto = ntohs(*(unsigned short *)(data + VR_ETHER_PROTO_OFF));
    while (eth_proto == VR_ETH_PROTO_VLAN) {
        if (pkt_len < (pull_len + sizeof(*vlan)))
            return -1;
        vlan = (struct vr_vlan_hdr *)(data + pull_len);
...
vr_pkt_type(struct vr_packet *pkt)
{
    ...
    pull_len = vr_get_l3_hdr_offset_from_eth(eth, pkt_head_len(pkt),
                                             &eth_proto);
    if (pull_len < 0)
        return -1;
            ...
            struct vr_forwarding_md *fmd)
{
    ...
    /* Got to L3 header for L3 processing */
    pull_len = pkt_get_network_header_off(pkt) - pkt_head_space(pkt);
