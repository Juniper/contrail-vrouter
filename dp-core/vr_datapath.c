        ...
        struct vr_forwarding_md *fmd, unsigned char *dmac)
{
    ...
    if (!eth) {
        DS_LOG(VP_DROP_PUSH, pkt, VR_DATAPATH_C, __LINE__);
                      ...
                      struct vr_forwarding_md *fmd, unsigned char *eth_dmac)
{
    ...
    case MR_DROP:
        DS_LOG(VP_DROP_INVALID_ARP, pkt, VR_DATAPATH_C, __LINE__);
                    ...
                    struct vr_forwarding_md *fmd)
{
    ...
    DS_LOG(VP_DROP_INVALID_IF, pkt, VR_DATAPATH_C, __LINE__);
        ...
        unsigned short trap_reason, void *trap_param)
{
    ...
    if (router->vr_agent_if && router->vr_agent_if->vif_send) {
        ...
        DS_LOG(VP_DROP_TRAP_NO_IF, pkt, VR_DATAPATH_C, __LINE__);
...
vr_reinject_packet(struct vr_packet *pkt, struct vr_forwarding_md *fmd)
{
    ...
    if (pkt->vp_nh) {
        ...
        if (!(pkt->vp_nh->nh_flags & NH_FLAG_VALID)) {
            DS_LOG(VP_DROP_INVALID_NH, pkt, VR_DATAPATH_C, __LINE__);
            vr_pfree(pkt, VP_DROP_INVALID_NH);
            return 0;
        }
...
vr_pbb_decode(struct vr_packet *pkt, struct vr_forwarding_md *fmd)
{
    ...
    if (pbb_size <= 0) {
        DS_LOG(VP_DROP_INVALID_PACKET, pkt, VR_DATAPATH_C, __LINE__);
        vr_pfree(pkt, VP_DROP_INVALID_PACKET);
        return decode_error;
    }

    if (!pkt_pull(pkt, pbb_size)) {
        DS_LOG(VP_DROP_PULL, pkt, VR_DATAPATH_C, __LINE__);
