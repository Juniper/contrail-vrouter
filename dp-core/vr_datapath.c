        ...
        struct vr_route_req *rt, unsigned char *dmac)
{
    ...
    if (from_fabric) {
         ...
         * of ENCAP type (to_gateway will be true if the nexthop is ENCAP and
         * the device type is agent). we should also proxy if the request is
         * for a host in vcp port. In all other cases, we should proxy only if
         *
