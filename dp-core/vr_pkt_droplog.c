...
static void
vr_pkt_drop_log_get(unsigned int rid, short core)
{
    ...
    if(vr_pkt_droplog_sysctl_en == 1) {
        ...
        if(vr_pkt_droplog_buf_en == 1) {
            ...
            response->vdl_log_idx = stats_index;
            ...
            if(cur_idx || (router->vr_pkt_drop->vr_pkt_drop_log[core][nxt_idx].timestamp != 0)) {
                if(cur_idx > stats_index) {
                    if(cur_idx - stats_index > VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ) {
                        pkt_buffer_size = VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ;
                        stats_index += VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ;
                        response->vdl_pkt_droplog_repeat = 1;

                    }
                    else {
                        pkt_buffer_size = cur_idx - stats_index;
                        stats_index += pkt_buffer_size;
                    ...
                    local_idx = cur_idx - stats_index;
                }
                else {
                    /* Below case would be entered when the circular buffer is overflown.
                     * Tries to fetch log backwards. i.e from 0 to (cur_idx+1) */
                    if(vr_pkt_droplog_bufsz - stats_index > VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ) {
                        pkt_buffer_size = VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ;
                        stats_index += VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ;
                        response->vdl_pkt_droplog_repeat = 1;
                    }
                    else {
                        pkt_buffer_size = vr_pkt_droplog_bufsz - stats_index;
                        stats_index += pkt_buffer_size;
                        response->vdl_pkt_droplog_repeat = 0;
                    }
                    local_idx = vr_pkt_droplog_bufsz - (stats_index - cur_idx);
...
vr_pkt_drop_log_req_process(void *s_req)
{
    ...
    vr_pkt_drop_log_get(req->vdl_rid, (req->vdl_core - 1));
