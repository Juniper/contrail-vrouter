...
vr_hugepage_config_process(void *s_req)
{
    ...
    router = vrouter_get(req->vhp_rid);
    ...
    if (vr_huge_page_config) {
        ...
        vr_printf("Vrouter: huge page config return status %d\n", ret);
    }
    ...
    if (ret != -EEXIST) {
        ...
        vr_printf("Vrouter: huge page config module return status %d\n", mret);
    }
    ...
    vr_printf("Vrouter: huge page config ret val %d resp code %d\n",
            ret, hcfg_resp.vhp_resp);
