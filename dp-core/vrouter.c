...
vr_logger_conf_get_process(void *s_req)
{
    ...
    for(i = 1;i < VR_NUM_MODS;i++) {
        resp->vo_log_mod_level[i] = logger.module[i].level;
        resp->vo_log_mod_type = logger.module[i].console;
...
vr_logger_conf_add_process(void *s_req)
{
    ...
    if(req->vo_log_mod_level_size != 0) {
        logger.module[module].level = req->vo_log_mod_level[module];
        logger.module[module].console = req->vo_log_mod_type;
    }

    if(req->vo_log_mod_len_size != 0) {
        level = req->vo_level;
        if(req->vo_log_mod_len[module * VR_NUM_LEVELS+level] != logger.module[module].level_info[level].log_size) {
            resize_module_buf(module, level, req->vo_log_mod_len[module * VR_NUM_LEVELS + level]);
        }
        logger.module[module].level_info[level].log_size = req->vo_log_mod_len[module * VR_NUM_LEVELS + level];
    }
