...
vrouter_ops_add_process(void *s_req)
{
    ...
    if(req->vo_log_mod_len_size != 0) {
        ...
        if(req->vo_resize_buf[mod*VR_NUM_LEVELS+lev] == 1) {
            resize_module_buf(mod, lev, req->vo_log_mod_len[mod*VR_NUM_LEVELS+lev], req->vo_rid);
        }
        log_ctrl[mod].entries[lev] = req->vo_log_mod_len[mod*VR_NUM_LEVELS+lev]/VR_LOG_ENTRY_LEN;
    }
