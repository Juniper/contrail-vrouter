           ...
           struct vr_forwarding_md *fmd)
{
    ...
    if (vr_ip_is_ip6(ip)) {
        ...
        family = AF_INET;
        if (!ip->ip_ttl) {
            DS_LOG(VP_DROP_TTL_EXCEEDED, pkt, 0, VR_PROTO_IP_C, __LINE__);
        ...
        DS_LOG(VP_DROP_INVALID_PROTOCOL, pkt, 0, VR_PROTO_IP_C, __LINE__);
        ...
        struct vr_forwarding_md *fmd)
{
    ...
    DS_LOG(0,pkt, 0,VR_PROTO_IP_C,__LINE__);
             ...
             struct vr_forwarding_md *fmd)
{
    ...
    if (vr_perfp && vr_pull_inner_headers_fast) {
        ...
        if (ret == PKT_RET_ERROR) {
            DS_LOG(VP_DROP_CKSUM_ERR, pkt, 0, VR_PROTO_IP_C, __LINE__);
    ...
    udph = (struct vr_udp *)vr_pheader_pointer(pkt, sizeof(struct vr_udp),
                                                &udp);
    if (udph == NULL) {
        DS_LOG(VP_DROP_MISC, pkt, 0, VR_PROTO_IP_C, __LINE__);
        ...
        struct vr_forwarding_md *fmd)
{
    ...
    if (unhandled) {
        ...
        if (!(ip = (struct vr_ip *)pkt_push(pkt, hlen))) {
            ...
            DS_LOG(drop_reason, pkt, 0, VR_PROTO_IP_C, __LINE__);
            goto drop_pkt;
        }
...
drop:
    DS_LOG(VP_DROP_FLOW_NAT_NO_RFLOW, pkt, 0, VR_PROTO_IP_C, __LINE__);
                    ...
                    struct vr_forwarding_md *fmd)
{
    ...
    if (ret < 0) {
        if (!vr_ip_transport_header_valid(ip) && vr_enqueue_to_assembler) {
            ...
            DS_LOG(VP_DROP_MISC, pkt, flow_p, VR_PROTO_IP_C, __LINE__);
            vr_pfree(pkt, VP_DROP_MISC);
        }
        return FLOW_CONSUMED;
    }
