    ...
    if(!req->vdu_buff_table_id) {
        ...
        for(i = 0; i < VR_INFO_MSG_BUF_TABLE; i++) {
            ...
            if(vr_info_buff_table_p[i].buff == NULL) {
                 ...
                 * registered table */
                if(req->vdu_msginfo ==
                        users_cb_reg[req->vdu_msginfo - 1].msginfo) {
                    ret = users_cb_reg[req->vdu_msginfo - 1].cb_fn(&msg_req);
                }
                else {
                ...
                if(msg_req.outbuf == NULL || msg_req.outbuf_len == 0) {
        ...
        if(vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buff != NULL) {
            buff_sz = vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buf_len;
        }
        else {
    ...
    if(buff_sz > max_buff_sz) {
        ...
        while(req->vdu_marker <= (buff_sz/max_buff_sz)) {
            ...
            snprintf(resp.vdu_proc_info, (resp.vdu_proc_info_size),
                    (vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buff +
...
exit_get:
    if(vr_info_last_buf) {
        if(vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buff != NULL) {
            vr_free(vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buff,
                    VR_INFO_REQ_OBJECT);
            vr_info_buff_table_p[resp.vdu_buff_table_id - 1].buff = NULL;
        }
        else
