...
static char *vr_info_buff_table_p[VR_INFO_MSG_BUF_TABLE];
...
vr_info_req_process(void *s_req)
{
    ...
    if(!req->vdu_buff_table_id) {
        ...
        for(i = 0; i < VR_INFO_MSG_BUF_TABLE; i++) {
            /* Iterate through message buffer table and find the free entry */
            if(vr_info_buff_table_p[i] == NULL) {
                ...
                for(i = 0; i <= VR_INFO_MAX_CALLBACK; i++ ) {
                    ...
                    if(req->vdu_msginfo == users_cb_reg[i].msginfo) {
                        ret = users_cb_reg[i].cb_fn(&msg_req.inbuf,
                                ...
                                &msg_req.outbuf_len, req->vdu_outbufsz);
                        break;
                    }
                    /* Reached end of loop */
                    if(i == VR_INFO_MAX_CALLBACK) {
                ...
                vr_info_buff_table_p[i] = msg_req.outbuf;
        ...
        buff_sz = strlen(vr_info_buff_table_p[resp.vdu_buff_table_id - 1]);
    ...
    if(buff_sz > max_buff_sz) {
        ...
        while(req->vdu_marker <= (buff_sz/max_buff_sz)) {
            ...
            snprintf(resp.vdu_proc_info, (resp.vdu_proc_info_size),
                    (vr_info_buff_table_p[resp.vdu_buff_table_id - 1] +
...
exit_get:
    if(vr_info_last_buf) {
        if(vr_info_buff_table_p[resp.vdu_buff_table_id - 1] != NULL) {
            vr_free(vr_info_buff_table_p[resp.vdu_buff_table_id - 1],
                    VR_INFO_REQ_OBJECT);
            vr_info_buff_table_p[resp.vdu_buff_table_id - 1] = NULL;
        }
        else
