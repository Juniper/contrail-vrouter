...
vr_drop_stats_get(short is_filtered)
{
    ...
    if (is_filtered) {
        if (router->vr_drop_stats_filter) {
            ptr_hldr = router->vr_pdrop_filter_stats;
        }
        else {
        ...
        ptr_hldr = router->vr_pdrop_stats;
    }

    for (cpu = 0; cpu < vr_num_cpus; cpu++) {

        stats_block = (struct vr_drop_stats *)ptr_hldr[cpu];
...
static void
vr_clean_up_stats(uint64_t **free_ptr)
{
    unsigned int i;
...
vr_pkt_drop_stats_exit(struct vrouter *router,short clean_all)
{
    ...
    if (router->vr_drop_stats_filter) {
        vr_clean_up_stats(router->vr_pdrop_filter_stats);
        router->vr_pdrop_filter_stats=NULL;
...
vr_pkt_drop_stats_init(struct vrouter *router)
{
    ...
    if (router->vr_drop_stats_filter) {
        if (router->vr_pdrop_filter_stats)
            return 0;
        router->vr_pdrop_filter_stats = vr_zalloc(size);
        ptr_hdlr = router->vr_pdrop_filter_stats;
        clean_all = 0;
    }
    else {
        ...
        ptr_hdlr = router->vr_pdrop_stats;
    }
    if (!ptr_hdlr) {
...
vr_pkt_drop_stats_reset(struct vrouter *router)
{
    ...
    if (router->vr_pdrop_filter_stats) {
        for (i = 0; i < vr_num_cpus; i++) {
            if (router->vr_pdrop_filter_stats[i])
                memset(router->vr_pdrop_stats[i], 0, VP_DROP_MAX * sizeof(uint64_t));
        }
    }
    return;
...
void print_filter( vr_drop_stats_filter_req *request)
{
...
void
print_tuple(struct vr_registered_tuple *input)
{
...
void
vr_drop_stats_filter_req_process(void *register_req )
{
    short clean_all = 0;
    vr_response response;
    vr_drop_stats_filter_req *request = (vr_drop_stats_filter_req *)register_req;
    struct vrouter *router = vrouter_get(0);
    ...
    if (request->h_op == SANDESH_OP_ADD) {
        ...
        if (router->vr_drop_stats_filter) {
            response.resp_code = -FILTER_ALREADY_REGISTERED;
            goto send_response;
            return;
        }

        router->vr_drop_stats_filter = vr_zalloc(sizeof(vr_drop_stats_filter_req));
        if (!router->vr_drop_stats_filter) {
            vr_send_response(-ENOMEM);
            return;
        }

        memcpy(router->vr_drop_stats_filter, request,sizeof(vr_drop_stats_filter_req));
    ...
    else if (request->h_op == SANDESH_OP_DELETE)
    {
        ...
        vr_drop_stats_filter_req *filter_response ;
        if ((filter_response = router->vr_drop_stats_filter) != NULL) {
            vr_message_response(VR_DROP_STATS_FILTER_OBJECT_ID,filter_response,0);
        }
        else {
            response.resp_code = -NO_FILTER_REGISTERED;
            goto send_response;
...
static int
is_matched_with_filter(struct vrouter *router, struct vr_registered_tuple *input)
{
    if (!input)
        return 0;
        ...
        struct vr_registered_tuple *tuple)
{
    ...
    struct vr_tcp *tcphdr = NULL;
    struct vr_udp *udphdr = NULL;
    ...
    if (iph->ip_proto == VR_IP_PROTO_TCP) {
        tcphdr = (struct vr_tcp *)((char *)iph + hlen);
        if (tcphdr) {
        ...
        udphdr = (struct vr_udp *)((char *)iph + hlen);
        if (udphdr) {
...
void
vr_set_pkt_filter(struct vrouter *router, struct vr_packet *pkt, short vrf)
{

    if (router->vr_drop_stats_filter) {
        struct vr_registered_tuple tuple;
        if (vr_make_tuple_from_pkt(pkt,vrf, &tuple)) {
            if (is_matched_with_filter(router, &tuple))
                pkt->is_matched_filter = true;
            else
                pkt->is_matched_filter = false;
        }
    }
    else {
        pkt->is_matched_filter = false;
