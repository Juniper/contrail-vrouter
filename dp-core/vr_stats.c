...
unsigned int vr_config_drop_stats_log_buffer_support = 1;
...
vr_drop_stats_log_get(unsigned int rid,short core,int index)
{
    ...
    {
        if(vr_config_drop_stats_log_buffer_size <= VR_DROP_STATS_MAX_ALLOWED_BUFFER_SIZE )
        {
            response->vds_drop_stats_log_arr_size = vr_config_drop_stats_log_buffer_size* sizeof(struct vr_drop_stats_log_st);
            
        }
        else
        {
            response->vds_drop_stats_log_arr_size = VR_DROP_STATS_MAX_ALLOWED_BUFFER_SIZE * sizeof(struct vr_drop_stats_log_st);
        }
        ...
        response->vds_drop_stats_support = vr_config_drop_stats_log_buffer_support;
...
vr_pkt_drop_stats_exit(struct vrouter *router)
{
    ...
    vr_free(router->vr_drop_stats_log_circular_buffer,VR_DROP_STATS_LOG_OBJECT);
    router->vr_drop_stats_log = NULL;
    router->vr_drop_stats_log_circular_buffer = NULL;
#endif
...
vr_pkt_drop_stats_init(struct vrouter *router)
{
    ...
    if(vr_config_drop_stats_log_buffer_support == 1)
    {
        ...
        router->vr_drop_stats_log_circular_buffer = vr_zalloc(size, VR_DROP_STATS_LOG_OBJECT);
        if (!router->vr_drop_stats_log_circular_buffer) {
            vr_module_error(-ENOMEM, __FUNCTION__,
                    __LINE__, size);
            goto cleanup;
        }
