...
static void
vr_drop_stats_add_stats(vr_drop_stats_req *response, uint64_t *stats)
{
    if (!response || !stats)
        return;
...
void
vr_drop_stats_add_vif_stats(vr_drop_stats_req *response,
                                struct vr_interface *vif)
{
    ...
    vr_drop_stats_add_stats(response, stats);
...
vr_drop_stats_get(unsigned int rid, short core)
{
    ...
    if (core == -1) {
        /* summed up stats */
        for (cpu = 0; cpu < vr_num_cpus; cpu++) {
            vr_drop_stats_add_stats(response, router->vr_pdrop_stats[cpu]);
        }
    } else if (core < vr_num_cpus) {
        /* stats for a specific core */
        vr_drop_stats_add_stats(response, router->vr_pdrop_stats[core]);
    }
