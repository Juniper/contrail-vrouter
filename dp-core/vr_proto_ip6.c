               ...
               struct vr_forwarding_md *fmd)
{
    ...
    DS_LOG(0, pkt, 0, VR_PROTO_IP6_C, __LINE__);
                    ...
                    struct vr_forwarding_md *fmd)
{
    ...
    if (ret < 0) {
        if (!vr_ip6_transport_header_valid(ip6) && vr_enqueue_to_assembler) {
            ...
            DS_LOG(VP_DROP_MISC, pkt, flow_p, VR_PROTO_IP6_C, __LINE__);
             ...
             struct vr_forwarding_md *fmd)
{
    ...
    if (!pkt_pull(pkt, sizeof(struct vr_ip6))) {
        DS_LOG(VP_DROP_PULL, pkt, 0, VR_PROTO_IP6_C, __LINE__);
    ...
    if (!pkt_push(pkt, sizeof(struct vr_ip6))) {
        DS_LOG(VP_DROP_PUSH, pkt, 0, VR_PROTO_IP6_C, __LINE__);
        ...
        unsigned char *eth_dmac)
{
    ...
    case MR_DROP:
        DS_LOG(VP_DROP_INVALID_ARP, pkt, 0, VR_PROTO_IP6_C, __LINE__);
...
drop:
    DS_LOG(VP_DROP_INVALID_PACKET, pkt, 0, VR_PROTO_IP6_C, __LINE__);
    vr_pfree(pkt, VP_DROP_INVALID_PACKET);
    return handled;
}
