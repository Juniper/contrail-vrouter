...
__vr_vrf_table_entry_del(struct vrouter *router, unsigned int index)
{
    ...
    if (vrf_table_entry->vrf_flags & VRF_FLAG_HBS_L_VALID) {
        hbs_l = vrf_table_entry->hbs_l_vif;
        vrouter_put_interface(hbs_l);
        vrf_table_entry->hbs_l_vif = NULL;
    }

    if (vrf_table_entry->vrf_flags & VRF_FLAG_HBS_R_VALID) {
...
vr_vrf_table_entry_add(vr_vrf_req *req)
{
    ...
    if (req->vrf_flags & VRF_FLAG_HBS_L_VALID) {
        if (req->vrf_hbfl_vif_idx == -1) {
            /* Request to clear the field */
            vrf_table_entry->vrf_flags &= ~VRF_FLAG_HBS_L_VALID;
            if (old_hbs_l) {
    ...
    if (req->vrf_flags & VRF_FLAG_HBS_R_VALID) {
        if (req->vrf_hbfr_vif_idx == -1) {
            /* Request to clear the field */
            vrf_table_entry->vrf_flags &= ~VRF_FLAG_HBS_R_VALID;
            if (old_hbs_r) {
