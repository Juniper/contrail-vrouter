        ...
        int8_t *dst_mac)
{
    ...
    if (dst_mac) {
        struct vr_eth *eth = pkt_data(pkt);
                    ...
                    struct vr_forwarding_md *fmd)
{
    ...
    for (i = 0; i < nh->nh_component_cnt; i++) {
        ...
        if (dir_nh->nh_flags & NH_FLAG_L3_VXLAN)
            dst_mac = dir_nh->nh_pbb_mac;
        /*
