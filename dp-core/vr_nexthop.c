                       ...
                       unsigned int dip)
{
    ...
    if (vr_get_udp_src_port) {
        udp_src_port = vr_get_udp_src_port(pkt, fmd, vrf, pkt_data(pkt));
        if (udp_src_port == 0) {
            return false;
        }
    }
                   ...
                   struct vr_nexthop *nh, struct vr_forwarding_md *fmd)
{
    ...
    unsigned short l2_offset, reason = VP_DROP_PUSH;
    struct vr_packet *tmp_pkt;
    struct vr_df_trap_arg trap_arg;
    void *l2_hdr;
    ...
    if (vr_get_udp_src_port) {
        l2_hdr = NULL;
        if (pkt->vp_flags & VP_FLAG_L2_PAYLOAD) {
            l2_offset = pkt->vp_data;
            if (pkt->vp_flags & VP_FLAG_MULTICAST) {
                l2_offset += VR_VXLAN_HDR_LEN + VR_L2_MCAST_CTRL_DATA_LEN;
            }
            l2_hdr = pkt_data_at_offset(pkt, l2_offset);
            if (!l2_hdr) {
                reason = VP_DROP_PULL;
                goto send_fail;
            }
        }
        udp_src_port = vr_get_udp_src_port(pkt, fmd, vrf, l2_hdr);
        if (udp_src_port == 0) {
            reason = VP_DROP_PULL;
            goto send_fail;
        }
