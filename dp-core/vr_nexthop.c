                     ...
                     struct vr_forwarding_md *fmd)
{
    ...
    // For 5.1, source is outside contrail and only <*,G> is supported.
    ...
    if (tun_src & NH_FLAG_VALIDATE_MCAST_SRC) {
        ...
        pkt_src = PKT_SRC_EDGE_REPL_TREE;
        // In this case ethernet header pointer need no be adjusted.
        pull_header = false;
    }
    ...
    for (i = 0; i < nh->nh_component_cnt; i++) {
        ...
        if (dir_nh->nh_flags & NH_FLAG_COMPOSITE_ENCAP) {
            ...
            if (!pull_header) {
                ...
                // other computes. Assumption that multicast data packet was
