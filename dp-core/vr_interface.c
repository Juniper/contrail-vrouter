...
{
    DS_LOG(VP_DROP_INTERFACE_TX_DISCARD, pkt, 0, VR_INTERFACE_C, __LINE__);
...
{
    DS_LOG(VP_DROP_INTERFACE_RX_DISCARD, pkt, 0, VR_INTERFACE_C, __LINE__);
...
vif_drop_pkt(struct vr_interface *vif, struct vr_packet *pkt, bool input)
{
    ...
    DS_LOG(VP_DROP_INTERFACE_DROP, pkt, 0, VR_INTERFACE_C, __LINE__);
        ...
        unsigned short vlan_id __attribute__unused__)
{
    ...
    if (!hdr || !pkt_pull(pkt, sizeof(*hdr))) {
        stats->vis_ierrors++;
        DS_LOG(VP_DROP_PULL, pkt, 0, VR_INTERFACE_C, __LINE__);
    ...
    case AGENT_CMD_SWITCH:
        ...
        if (!vif) {
            stats->vis_ierrors++;
            DS_LOG(VP_DROP_INVALID_IF, pkt, 0, VR_INTERFACE_C, __LINE__);
    ...
    default:
        DS_LOG(VP_DROP_INVALID_PACKET, pkt, 0, VR_INTERFACE_C, __LINE__);
                ...
                void *ifspecific)
{
    ...
    if (truncate || (pkt_head_space(pkt) < head_space)) {
        ...
        if (pkt_c) {
            DS_LOG(VP_DROP_DUPLICATED, pkt, 0, VR_INTERFACE_C, __LINE__);
...
drop:
    stats->vis_oerrors++;
    DS_LOG(VP_DROP_PUSH, pkt, 0, VR_INTERFACE_C, __LINE__);
