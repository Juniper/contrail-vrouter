        ...
        struct vr_bond_slaves_info *bond_slaves_info)
{
    ...
    if (req->vifr_name) {
        if ( (NULL != bond_slave_params) && (slave_index >= 0) ) {
            if (req->vifr_type == VIF_TYPE_PHYSICAL) {
                snprintf(req->vifr_name, VR_INTERFACE_NAME_LEN - 1, "vif%d/%d/slave%d", req->vifr_rid, req->vifr_idx, slave_index);
            } else {
    ...
    if ( NULL == bond_slave_params ) {
        ...
        if (!bond_slave_params->vif_stats) {
            ...
            /* call host callback if available */
            if (hif_ops->hif_stats_update) {
                hif_ops->hif_stats_update(intf, core, bond_slave_params->port_id, bond_slave_params->vif_stats);
            }
...
vr_interface_get(vr_interface_req *req)
{
    ...
    if (req->vifr_name) {
        if (NULL != (slave_substr = strstr(req->vifr_name, "slave"))) {
            ...
            if ( (vif->vif_type == VIF_TYPE_PHYSICAL) && (vif->vif_type == VIF_TYPE_MONITORING) ) {
                if ((ret = hif_ops->hif_get_slaves_info(vif_os, (unsigned)(req->vifr_core - 1), &bond_slaves_info))) {
    ...
    ret = vr_interface_make_req(vif_resp, vif, (unsigned)(req->vifr_core - 1), slave_index, &bond_slaves_info);
...
vr_interface_dump(vr_interface_req *r)
{
    ...
    for (i = (unsigned int)(r->vifr_marker + 1);
        ...
        if (vif) {
            ...
            if ( (vif->vif_type == VIF_TYPE_PHYSICAL) || (vif->vif_type == VIF_TYPE_MONITORING) ) {
                if ((ret = hif_ops->hif_get_slaves_info(vif_os, (unsigned)(r->vifr_core - 1), &bond_slaves_info))) {
            ...
            if (vif->vif_type == VIF_TYPE_PHYSICAL) {
                for (slave_idx = 0; slave_idx < bond_slaves_info.nbr_bond_slaves; slave_idx++) {
                    /* zero vifr_core means to sum up all the per-core stats */
                    vr_interface_make_req(resp, vif, (unsigned)(r->vifr_core - 1), slave_idx, &bond_slaves_info);
