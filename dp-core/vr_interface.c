...
vr_interface_make_req(vr_interface_req *req, struct vr_interface *vif)
{
    ...
    __vr_interface_make_req(req, vif);
...
vr_interface_get(vr_interface_req *req)
{
    ...
    if (vif) {
        ...
        ret = vr_interface_make_req(resp, vif);
    } else
        ret = -ENOENT;
...
vif_fat_flow_add(struct vr_interface *vif, vr_interface_req *req)
{
    ...
    uint16_t port,
             old_combo_flow_config_sizes[VIF_FAT_FLOW_MAXPROTO_INDEX] = { 0 };
    uint16_t *vif_old_combo_flow_config[VIF_FAT_FLOW_MAXPROTO_INDEX] = { NULL };
     ...
     */
    memcpy(old_combo_flow_config_sizes, vif->vif_fat_flow_config_size,
            sizeof(old_combo_flow_config_sizes));
    memset(vif->vif_fat_flow_config_size, 0,
            sizeof(vif->vif_fat_flow_config_size));

    memcpy(vif_old_combo_flow_config, vif->vif_fat_flow_config,
            sizeof(vif_old_combo_flow_config));
    ...
    for (i = 0; i < req->vifr_fat_flow_protocol_port_size; i++) {
         ...
         */
        for (j = 0; j < old_combo_flow_config_sizes[proto_index]; j++) {
            if (vif_old_combo_flow_config[proto_index][j] == port) {
                /* already present in the old configuration. hence no additon */
                vif_old_combo_flow_config[proto_index][j] = 0;
                add = false;
                break;
            }
        }
    ...
    for (i = 0; i < VIF_FAT_FLOW_MAXPROTO_INDEX; i++) {
        for (j = 0; j < old_combo_flow_config_sizes[i]; j++) {
            if (vif_old_combo_flow_config[i] && vif_old_combo_flow_config[i][j])
                __vif_fat_flow_delete(vif, i, vif_old_combo_flow_config[i][j]);
        }

        if (old_combo_flow_config_sizes[i] && vif_old_combo_flow_config[i]) {
            vr_free(vif_old_combo_flow_config[i]);
            vif_old_combo_flow_config[i] = NULL;
        }
    }
