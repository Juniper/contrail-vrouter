...
extern void vr_drop_stats_add_vif_stats(vr_drop_stats_req *, struct vr_interface *);
...
static uint64_t
vr_interface_pkt_drops(struct vr_interface *vif)
{
    uint8_t *count, cpu;
    int stats_index;
        ...
        unsigned int core)
{
    ...
    req->vifr_dpackets = vr_interface_pkt_drops(intf);
    return;
}
...
vr_interface_get(vr_interface_req *req)
{
    ...
    vrresp.h_op = SANDESH_OP_RESPONSE;
    mm.vr_mm_object_type[obj_cnt] = VR_RESPONSE_OBJECT_ID;
    mm.vr_mm_object[obj_cnt] = &vrresp;
    obj_cnt++;

    router = vrouter_get(req->vifr_vrf);
    if (!router) {
        ret = -ENODEV;
    ...
    ret = vr_interface_make_req(resp, vif, (unsigned)(req->vifr_core - 1));
    if (ret < 0)
        goto generate_response;
    ...
    if (req->vifr_flags & VIF_FLAG_GET_DROP_STATS) {
        ...
        vr_drop_stats_add_vif_stats(drop_resp, vif);
