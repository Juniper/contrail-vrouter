        ...
        vr_interface_req *vifr __attribute__((unused)))
{
    ...
    /*
     * do not start claiming packets as soon as the interface is added
     * and if the interface is set to be associated with a vhost interface.
     * there are two cases: in a fresh boot, the interface is anyways
     * unusable till vhost is added. in a case when agent restarts, resets
     * and adds the interface back, the moment reset happens, the physical
     * interface is directly bridged to vhost and traffic will bypass the
     * vrouter. We will blackout traffic if we start tapping packets here
     * since the vhost interface corresponding to the physical interface has
     * not been added
     */
