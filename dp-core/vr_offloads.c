...
vr_offloads_init(struct vrouter *router)
{
    ...
    if (!offload_tags) {
        ...
        for (esize = sizeof(struct vr_offload_tag); VR_SINGLE_ALLOC_LIMIT % esize; esize++);
        offload_tags = vr_btable_alloc(vr_nexthops, esize);
        if (!offload_tags)
            return -ENOMEM;
    }
    ...
    if (!offload_flows) {
        /* Round up to the next divisor */
        for (esize = sizeof(struct vr_offload_flow); VR_SINGLE_ALLOC_LIMIT % esize; esize++);
        offload_flows = vr_btable_alloc(vr_flow_entries + vr_oflow_entries, esize);
        if (!offload_flows) {
