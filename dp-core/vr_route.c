...
error:
    if(ret == 0) {
        VR_LOG_RT(info, req->rtr_prefix_size, req->rtr_prefix, "OP: %d family: %d rid: %d label: %d nh_id: %d rtr_replace_plen: %d prefix:",
        req->h_op, req->rtr_family, req->rtr_rid, req->rtr_label, req->rtr_nh_id, req->rtr_replace_plen);
    }
    else{
        VR_LOG_RT(error, req->rtr_prefix_size, req->rtr_prefix, "OP: %d family: %d rid: %d label: %d nh_id: %d Err code: %d prefix:",
...
vr_route_add(vr_route_req *req)
{
    ...
    if(ret != -ENOENT) {
        VR_LOG_RT(info, req->rtr_prefix_size, req->rtr_prefix, "OP: %d family: %d rid: %d label: %d nh_id: %d rtr_replace_plen: %d prefix:", req->h_op,
        req->rtr_family, req->rtr_rid, req->rtr_label, req->rtr_nh_id, req->rtr_replace_plen);
    }
    else {
        VR_LOG_RT(error, req->rtr_prefix_size, req->rtr_prefix, "OP: %d family: %d rid: %d label: %d nh_id: %d Err code: %d func: %s prefix: ",
        req->h_op, req->rtr_family, req->rtr_rid, req->rtr_label, req->rtr_nh_id, ret, __FUNCTION__);
    }
    vr_send_response(ret);
...
generate_error:
    vr_send_response(ret);
    if(ret != 0) {
        //VR_LOG_RT(error, "OP: %d family: %d rid: %d label: %d nh_id: %d Err code: %d prefix:",
         //req->h_op, req->rtr_family, req->rtr_rid, req->rtr_label, req->rtr_nh_id, ret);
    }
    return;
}
...
error_log:
    if(ret != 0) {
        VR_LOG_RT(error, req->rtr_req.rtr_prefix_size, req->rtr_req.rtr_prefix, "OP: %d family: %d rid: %d label: %d nh_id: %d Err code: %d prefix:",
         req->rtr_req.h_op, req->rtr_req.rtr_family, req->rtr_req.rtr_rid, req->rtr_req.rtr_label, req->rtr_req.rtr_nh_id, ret);
    }
    return ret;
}
