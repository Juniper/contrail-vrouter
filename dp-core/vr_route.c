...
error:
    if(ret == 0) {
        vr_log(MODULE_ROUTE, VR_INFO, "OP:%d fam:%d rid:%d lab:%d nh:%d rtr_rep:%d prefix:",
        ...
        vr_log(MODULE_ROUTE, VR_ERROR, "OP:%d fam:%d rid:%d lab:%d nh:%d Err:%d prefix:%s",
              req->h_op, req->rtr_family, req->rtr_rid, req->rtr_label, req->rtr_nh_id,
...
generate_error:
    vr_send_response(ret);
    if(ret != 0) {
        vr_log(MODULE_ROUTE, VR_ERROR, "OP:%d fam:%d rid:%d vrd:%d type:%d Err:%d",
