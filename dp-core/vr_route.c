...
vr_route_get(vr_route_req *req)
{
    ...
    /* Allow for debug comparison to check if matching entry is programmed on NIC
       (currently not implemented for route table).  Note due to differences in
       how route tables can be implemented when offloaded there is not a similar
       comparison available for route dump. */
    if (!ret)
