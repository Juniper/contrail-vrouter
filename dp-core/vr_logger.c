 ...
 * Retrieve log index
 */
#define GET_LOG_BUF_IDX(Module) \
    (router->vr_logger->vr_log_buf[Module].buf_idx%VR_LOG_MAX)

/*
 * Retrieve log
 */
#define GET_LOG_BUF(Module) \
...
int vr_log_get(unsigned int rid, int module, int index, int cur_index) {
    ...
    char *log = GET_LOG_BUF(module);
    int inc_index;
    if(cur_index == -1) {
        cur_index = GET_LOG_BUF_IDX(module);
    }
    int ret;
...
vr_log_req_process(void *s_req)
{
    ...
    vr_log_get(req->vdl_rid, module, index, cur_index);
}
