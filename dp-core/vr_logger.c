...
#define GET_LOG_LINE_IDX(module, log_level) \
    (logger.module[module].level_info[level].line_idx % \
    SIZE_TO_RECORDS(logger.module[module].level_info[log_level].log_size))

/*
 * Retrieve log
 */
#define GET_LOG_BUF(module, log_level) \
    (logger.module[module].level_info[level].buffer)
...
void vr_log_get(unsigned int rid, int module, int level, int index, int cur_index) {
    ...
    if (cur_index == -1) {
        cur_index = (GET_LOG_LINE_IDX(module, level)) * VR_LOG_ENTRY_LEN;
    }
...
void
vr_free_buffer(struct vrouter *router, void *arg)
{
    char *buf = (char *) arg;
    vr_free(buf, VR_LOG_OBJECT);
}
...
vr_log_req_process(void *s_req)
{
    ...
    if (req->vdl_clear_buf == 1) {
        ...
        vr_defer(router, vr_free_buffer, (void *) logger.module[module].level_info[level].buffer);
        logger.module[module].level_info[level].buffer =
        vr_zalloc(logger.module[module].level_info[level].log_size,
                  VR_LOG_OBJECT);
    }
...
inline void vr_log(int module, int level, char *fmt, ...) {
    ...
    if (VR_ALLOW_LOGGING(module, level)) {
        ...
        log = logger.module[module].level_info[level].buffer;
        line_idx = vr_sync_fetch_and_add_32u(&logger.module[module].level_info[level].line_idx, 1);
        line_idx %= SIZE_TO_RECORDS(logger.module[module].level_info[level].log_size);
