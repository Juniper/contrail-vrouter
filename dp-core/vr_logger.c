...
#define GET_LOG_BUF_IDX(module, level) \
    (router->vr_logger->vr_log_buf[module].buf_idx[level]%router->vr_logger->vr_log_buf[module].log_size[level])
 ...
 */
            for(j=1;j<VR_NUM_LEVELS;j++) {
                vr_log->vr_log_buf[0].log_size[j] = log_entries_flow[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[0].entries[j] = log_entries_flow[j];
                vr_log->vr_log_buf[0].buf[j] = vr_zalloc(vr_log->vr_log_buf[0].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[0].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[0].log_size[j]);
                    goto cleanup;
                }

                vr_log->vr_log_buf[1].log_size[j] = log_entries_interface[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[1].entries[j] = log_entries_interface[j];
                vr_log->vr_log_buf[1].buf[j] = vr_zalloc(vr_log->vr_log_buf[1].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[1].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[1].log_size[j]);
                    goto cleanup;
                }

                vr_log->vr_log_buf[2].log_size[j] = log_entries_mirror[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[2].entries[j] = log_entries_mirror[j];
                vr_log->vr_log_buf[2].buf[j] = vr_zalloc(vr_log->vr_log_buf[2].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[2].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[2].log_size[j]);
                    goto cleanup;
                }

                vr_log->vr_log_buf[3].log_size[j] = log_entries_nexthop[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[3].entries[j] = log_entries_nexthop[j];
                vr_log->vr_log_buf[3].buf[j] = vr_zalloc(vr_log->vr_log_buf[3].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[3].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[3].log_size[j]);
                    goto cleanup;
                }

                vr_log->vr_log_buf[4].log_size[j] = log_entries_qos[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[4].entries[j] = log_entries_qos[j];
                vr_log->vr_log_buf[4].buf[j] = vr_zalloc(vr_log->vr_log_buf[4].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[4].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[4].log_size[j]);
                    goto cleanup;
                }

                vr_log->vr_log_buf[5].log_size[j] = log_entries_route[j] * VR_LOG_ENTRY_LEN; //To be changed for loadtime parameters.
                log_ctrl[5].entries[j] = log_entries_route[j];
                vr_log->vr_log_buf[5].buf[j] = vr_zalloc(vr_log->vr_log_buf[5].log_size[j], VR_LOG_OBJECT);
                if(!vr_log->vr_log_buf[5].buf[j])
                {
                    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_log->vr_log_buf[5].log_size[j]);
                    goto cleanup;
                }
            }
    }
 ...
 */
    ...
    if(!response->vdl_vr_log) {
        vr_module_error(ENOMEM, __FUNCTION__, __LINE__, response->vdl_vr_log_size);
        goto exit_get;
    }
/*
...
vr_log_req_process(void *s_req)
{
    ...
    if(req->vdl_clear_buf == 1) {
        ...
        router->vr_logger->vr_log_buf[module].buf[log_level] = vr_zalloc(router->vr_logger->vr_log_buf[module].log_size[log_level], VR_LOG_OBJECT);
    }
...
inline void  VR_LOG(int module, int level, char *fmt) {
    ...
    if(VR_ALLOW_LOGGING(module, level)) {
        ...
        if(vr_log != NULL) {
            ...
            idx = vr_sync_fetch_and_add_32u(&vr_log->vr_log_buf[module].buf_idx[level], VR_LOG_ENTRY_LEN);
