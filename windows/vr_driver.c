...
doesIfCountedStringStartWithUnicodeString(PIF_COUNTED_STRING_LH s, PUNICODE_STRING prefix)
{
    if (s->Length >= prefix->Length) {
        uint8_t *a = (uint8_t*)s->String;
        uint8_t *b = (uint8_t*)prefix->Buffer;
...
{
    const PWSTR containerPrefix = L"Container";
    UNICODE_STRING containerPrefixUnicode;

    RtlInitUnicodeString(&containerPrefixUnicode, containerPrefix);
    return doesIfCountedStringStartWithUnicodeString(&NicParams->NicFriendlyName, &containerPrefixUnicode);
}
