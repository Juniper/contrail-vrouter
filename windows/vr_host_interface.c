...
void POC_ExtractOuterHeadersFromOriginalPacket(struct POC_FragmentationContext* pctx) {
    ...
    if (pctx->outer_ip_header->ip_proto == VR_IP_PROTO_GRE) {
        pctx->outer_headers_size += sizeof(struct vr_gre) + 4 /* MPLS */;
        ...
        // VXLAN port assigned by IANA is 4789. At this stage we don't have
        // better method to distinguish between VXLAN and MPLS.
        if (outer_udp_header->udp_dport == 4789) {
            pctx->outer_headers_size += 8; // VXLAN
        } else {
            pctx->outer_headers_size += 4; // MPLS
        }
    }
}
...
void POC_ExtractInnerHeadersFromOriginalPacket(
    ...
    pctx->total_payload_size = pctx->outer_ip_header->ip_len - pctx->inner_payload_offset;

    // Extract inner fragmentation flags and offset.
    pctx->inner_ip_mf = pctx->inner_ip_header->ip_frag_off & (1 << 2);
    pctx->inner_ip_byte_offset = pctx->inner_ip_header->ip_frag_off & ~7;
}
...
void POC_FixHeadersOfInnerFragmentedPacket(
    ...
    if (more_new_packets || pctx->inner_ip_mf) {
        fragment_inner_ip_header->ip_frag_off |= (1 << 2);
    }

    // Fix 'fragment offset' in inner IP header.
    fragment_inner_ip_header->ip_frag_off &= 7;
...
PNET_BUFFER_LIST POC_Fragment(struct vr_packet *pkt) {
    ...
    // What shall we do with original_nbl?
