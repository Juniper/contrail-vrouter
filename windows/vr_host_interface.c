...
void frg_initialize_fragmentation_context(
    ...
    PWIN_PACKET winPacket = GetWinPacketFromVrPacket(pkt);
    // TODO: it will be needed once code refactoring is merged.
...
bool frg_is_fragmentation_needed(struct FragmentationContext* pctx) {
    return vr_pkt_type_is_overlay(pctx->pkt->vp_type)
        && pctx->original_nbl->FirstNetBuffer->DataLength > pctx->mtu;
}

void frg_extract_outer_headers_from_original_packet(
        struct FragmentationContext* pctx) {
...
void frg_extract_inner_headers_from_original_packet(
        struct FragmentationContext* pctx) {
    ...
    // We assume that IP header has constant size (which is not true).
    // TODO: IPv6.
...
void frg_split_original_packet(struct FragmentationContext* pctx) {
    ...
    frg_fix_split_packet_metadata(pctx);
}

void frg_copy_headers_to_net_buffer(
        struct FragmentationContext* pctx,
        PNET_BUFFER nb) {
...
void frg_fix_headers_of_inner_fragmented_packet(
        struct FragmentationContext* pctx,
...
void frg_fix_headers_of_outer_fragmented_packet(
        struct FragmentationContext* pctx,
        unsigned char* headers) {
    struct vr_ip* fragment_inner_ip_header = (struct vr_ip*)(headers
        + pctx->outer_headers_size + sizeof(struct vr_eth));
...
void frg_fix_headers_of_fragmented_packet(struct FragmentationContext* pctx) {
    PNET_BUFFER cur_nb;
    PNET_BUFFER next_nb;
    ...
    for (cur_nb = pctx->fragmented_nbl->FirstNetBuffer;
        ...
        // TODO: check if NdisGetDataBuffer works here
        // unsigned char* headers = NdisGetDataBuffer(
        //     cur_nb, pctx->inner_payload_offset, NULL, 1, 0);
        unsigned char* headers = (unsigned char*) MmGetSystemAddressForMdlSafe(
            cur_nb->CurrentMdl, LowPagePriority | MdlMappingNoExecute)
            + cur_nb->CurrentMdlOffset;
        ASSERTMSG("fix_headers_of_fragmented_packet: headers == NULL",
            headers != NULL);
...
PNET_BUFFER_LIST fragment_packet_if_needed(struct vr_packet *pkt) {
...
__win_if_tx(struct vr_interface *vif, struct vr_packet *pkt)
{
    ...
    nbl = fragment_packet_if_needed(pkt);
