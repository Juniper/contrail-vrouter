...
void frg_extract_outer_headers_from_original_packet(
    ...
    if (pctx->outer_ip_header->ip_proto == VR_IP_PROTO_GRE) {
        ...
        if (vr_vxlan_udp_port(ntohs(outer_udp_header->udp_dport))) {
...
bool frg_fix_split_packet_metadata(struct FragmentationContext* pctx) {
...
unsigned char* frg_get_fragmented_packet_headers(PNET_BUFFER nb) {
    return (unsigned char*) MmGetSystemAddressForMdlSafe(
        nb->CurrentMdl, LowPagePriority | MdlMappingNoExecute)
        + nb->CurrentMdlOffset;
}

void frg_remove_fragmented_nbl(struct FragmentationContext* pctx) {
