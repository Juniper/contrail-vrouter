    ...
    struct FragmentationContext* pctx)
{
    ...
    pctx->outer_headers_size = sizeof(struct vr_eth) + outer_iph->ip_hl;
    if (outer_iph->ip_proto == VR_IP_PROTO_GRE) {
        ...
        struct vr_udp* outer_udph = (struct vr_udp*)((uint8_t *)outer_iph + outer_iph->ip_hl);
...
frg_fragment_offset(struct vr_ip* ip)
{
    ...
    struct FragmentationContext* pctx)
{
    ...
    pctx->inner_ip_frag_offset_in_bytes
        = frg_fragment_offset(pctx->inner_ip_header);
}
    ...
    unsigned short* byte_offset_for_next_inner_ip_header)
{
    ...
    *byte_offset_for_next_inner_ip_header
    ...
    unsigned short inner_ip_packet_length = sizeof(struct vr_ip);
    if (more_new_packets) {
