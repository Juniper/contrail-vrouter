...
#define MORE_FRAGMENTS_FLAG (1 << 13)
#define OFFSET_FLAGS_MASK ((1 << 13) - 1)
...
void POC_FixHeadersOfInnerFragmentedPacket(
    ...
    if (more_new_packets || pctx->inner_ip_mf) {
        fragment_inner_ip_header->ip_frag_off |= htons(MORE_FRAGMENTS_FLAG);
    }

    // Fix 'fragment offset' in inner IP header.
    fragment_inner_ip_header->ip_frag_off &= htons(OFFSET_FLAGS_MASK);
    ...
    *byte_offset_for_next_inner_ip_header
    ...
    fragment_inner_ip_header->ip_len = htons(pctx->inner_headers_size
