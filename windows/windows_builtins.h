...
{
    return _InterlockedExchangeAdd16(ptr, -((SHORT)val)) - val;
}

__forceinline UINT32
vr_sync_sub_and_fetch_32u(volatile UINT32 *ptr, UINT32 val)
{
    return InterlockedAdd(ptr, -((LONG)val));
}
...
{
    return InterlockedAdd64(ptr, -((LONGLONG)val));
}
...
{
    return InterlockedExchangeAdd64(ptr, (LONGLONG)val) + val;
}
...
{
    return InterlockedExchangeAdd(ptr, (LONG)val);
}

__forceinline UINT64
vr_sync_fetch_and_add_64u(volatile UINT64 *ptr, UINT64 val)
{
    return InterlockedExchangeAdd64(ptr, (LONGLONG)val);
}
...
{
    return InterlockedOr16(ptr, (SHORT)val);
}
...
{
    return InterlockedAnd16(ptr, (SHORT)val) & ((SHORT)val);
}

__forceinline UINT32
vr_sync_and_and_fetch_32u(volatile UINT32 *ptr, UINT32 val)
{
    return InterlockedAnd(ptr, (LONG)val) & ((LONG)val);
}
...
vr_sync_bool_compare_and_swap_8u(
    volatile UINT8 *ptr, UINT8 oldval, UINT8 newval)
{
    CHAR result = _InterlockedCompareExchange8(ptr, (CHAR)newval, (CHAR)oldval);
...
{
    LONG result = _InterlockedCompareExchange(ptr, (LONG)newval, (LONG)oldval);
