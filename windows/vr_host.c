...
win_pclone(struct vr_packet *pkt)
{
    ...
    PVR_PACKET_WRAPPER wrapper = GetWrapperFromVrPacket(pkt);
    PNET_BUFFER_LIST originalNbl = WinPacketToNBL(wrapper->WinPacket);
    ...
    PVR_PACKET_WRAPPER rightWrapper = ExAllocatePoolWithTag(NonPagedPoolNx, sizeof(*rightWrapper), VrAllocationTag);
    if (rightWrapper == NULL) {
        goto cleanup_both_nbls;
    }
    RtlZeroMemory(rightWrapper, sizeof(*rightWrapper));

    struct vr_packet *npkt = &rightWrapper->VrPacket;
    *npkt = *pkt;

    wrapper->WinPacket = WinPacketFromNBL(leftNbl);
    pkt->vp_cpu = (unsigned char)win_get_cpu();

    rightWrapper->WinPacket = WinPacketFromNBL(rightNbl);
