...
static struct option long_options[] = {
    ...
    [LACP_OPT_INDEX]    =   {"lacp",    no_argument,        &lacp_set,      1},
...
Usage()
{
    ...
    printf("--lacp|-l Show LACP sent/received packets from DPDK\n");
    exit(-EINVAL);
}
...
dpdkutils_req_process(void *s_req)
{
    ...
    vr_dpdk_info_req *resp = (vr_dpdk_info_req *)s_req;
...
{
    nl_cb.vr_dpdk_info_req_process = dpdkutils_req_process;
...
/* vr_get_dpdkutils API send request to vRouter(server) and uses
 * h_op = DUMP for message request.
 * The messages are sent through character buffer  and Sandesh has a limitation
 * of sending upto 4K(PAGE_SIZE).
 * */
static int
...
op_retry:
    ret = vr_send_dpdkutils_dump(cl, 0, dump_marker, buff_table_id, msginfo);
    if (ret < 0)
        return ret;
