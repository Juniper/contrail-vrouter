...
vr_route_req_process(void *s_req)
{
    ...
    if ((rt->rtr_family == AF_INET) ||
        (rt->rtr_family == AF_INET6)) {
            ...
            if (rt->rtr_mac_size) {
                    printf("%s(%d)", ether_ntoa((struct ether_addr
                                    *)(rt->rtr_mac)), rt->rtr_index);
            }
            else {
                printf("-");
            }
...
vr_route_op(void)
{
    ...
    if (cmd_op == SANDESH_OP_DUMP) {
        if ((cmd_family_id == AF_INET) || (cmd_family_id == AF_INET6)) {
            ...
            printf("Destination	           PPL        Flags        Label         Nexthop    Stitched MAC(Bridge Index)\n");
        } else {
            printf("Kernel L2 Bridge table %d/%d\n\n", req->rtr_rid, cmd_vrf_id);
            dump_legend(cmd_family_id);
            printf("Bridge Index     DestMac                          Flags       Label/VNID      Nexthop\n");
        }
    }
