...
class Flow(ObjectBase, vr_flow_req):
    ...
    def __repr__(self):
        ...
        return "Flow(sip:{} dip:{} sport:{} dport{})".format(
            self.fr_flow_sip_l, self.fr_flow_dip_l, self.fr_flow_sport, self.fr_flow_dport)
    ...
    def sync_and_add_reverse_flow(self):
        ...
        rflow.fr_flags = constants.VR_FLOW_FLAG_ACTIVE | constants.VR_RFLOW_VALID
    ...
    def get_fr_index(self):
        """Queries vrouter and returns fresp_index value from the response xml file"""
        return int(self.get('fresp_index'))

    def get_fr_gen_id(self):
        """Queries vrouter and returns fresp_gen_id value from the response xml file"""
