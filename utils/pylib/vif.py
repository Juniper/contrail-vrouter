...
class Vif(VTestObjectBase, VTestCommon, vr_interface_req):
    ...
    def __init__(
        ...
        if ip6_u is not None:
            self.vifr_ip6_u = ip6_u
        if ip6_l is not None:
    ...
    def send_and_receive_packet(self, tx_pkt_list, rx_pkt_list, receive_vif):
        ...
        self.create_pcap_req(tx_pkt_list, self.vifr_name, rx_pkt_list,
                             receive_vif, req_filename, output_pcap)
        # run the vtest cmd
...
class VirtualVif(Vif):
    ...
    def __init__(self, name, ipv4_str, mac_str, idx=0, ip6_u=None, ip6_l=None,
...
class AgentVif(Vif):
    def __init__(self, idx=0, *args, **kwargs):
        super(AgentVif, self).__init__(idx, 'unix', None, None, 0, 0,
                                       *args, **kwargs)
...
class VhostVif(Vif):
    def __init__(
            ...
            *args,
            **kwargs):
        super(VhostVif, self).__init__(idx, name, ipv4_str, mac_str, 0, 0,
                                       *args, **kwargs)
...
class FabricVif(Vif):
    def __init__(
            ...
            *args,
            **kwargs):
        super(FabricVif, self).__init__(idx, name, ipv4_str, mac_str, 0, 0,
                                        *args, **kwargs)
