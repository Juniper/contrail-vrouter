...
class NextHop(VTestObjectBase, vr_nexthop_req):
    ...
    def __init__(self, nh_type, nh_id, nh_family, nh_vrf, nh_flags,
        ...
        if VTestObjectBase.auto_nh_id_alloc is True:
...
class TunnelNextHopV4(NextHop):
    ...
    def __init__(self, encap_oif_id, encap, tun_sip, tun_dip,
                 nh_id=1, nh_vrf=0, nh_flags=vtconst.NH_FLAG_VALID,
                 encap_family=None, **kwargs):
...
class ReceiveL2NextHop(NextHop):
    """
    ReceiveL2NextHop class to create v4 tunnel nexthops
...
class ReceiveNextHop(NextHop):
    """
    ReceiveNextHop class to create v4 tunnel nexthops
