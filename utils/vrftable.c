...
static int get_set, hbfl_set, hbfr_set, vrf_set;
static int help_set, cmd_set;
static int vrf_op = -1;
static int vrf_index = -1, vrf_flags, hbfl_idx, hbfr_idx;
...
vrf_req_process(void *s_req)
{
    ...
    printf("    %7d    %4d", req->vrf_hbfl_vif_idx, req->vrf_hbfr_vif_idx);
...
op_retry:
    ...
    case SANDESH_OP_ADD:
        ret = vr_send_vrf_add(cl, 0, vrf_index,
                hbfl_idx, hbfr_idx, VRF_FLAG_VALID|VRF_FLAG_HBF_L_VALID|VRF_FLAG_HBF_R_VALID);
        break;
...
static struct option long_options[] = {
    ...
    [HBFL_OPT_INDEX]        =       {"hbf-l",   required_argument,  &hbfl_set,      1},
    [HBFR_OPT_INDEX]        =       {"hbf-r",   required_argument,  &hbfr_set,      1},
...
usage_internal()
{
    printf("Usage:      vrftable --create <vrf_idx> --hbf-l <vif index> --hbf-r <vif idx>\n");
    printf("            vrftable --delete <vrf_idx>\n");
    printf("\n");
    printf("--create    Create a vrf entry for <vrf_idx> with hbf left and hbf right vif index\n");
...
parse_long_opts(int opt_index, char *opt_arg)
{
    ...
    case HBFL_OPT_INDEX:
        hbfl_idx = strtoul(opt_arg, NULL, 0);
        if (errno)
            usage_internal();
        break;

    case HBFR_OPT_INDEX:
        hbfr_idx = strtoul(opt_arg, NULL, 0);
        if (errno)
            usage_internal();
        break;
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "bcdgl:r:v:s:",
            ...
            case 'l':
                hbfl_idx = atoi(optarg);
                hbfl_set = 1;
                break;

            case 'r':
                hbfr_idx = atoi(optarg);
                hbfr_set = 1;
                break;
    ...
    if ((vrf_op == SANDESH_OP_DUMP) ||
            (vrf_op == SANDESH_OP_GET)) {
        printf("VRF Table\n\n");
        printf("Flags: V=Valid, Hl=HBF Left Valid, Hr=HBF Right Valid\n\n");
        printf("   Vrf   Flags      HBF-L   HBF-R\n");
        printf("------------------------------------------------\n");
    }
