...
static int sock_dir_set, sock_port_set;
#ifdef _WIN32
static int guid_set;
#endif
...
Usage()
{
    ...
    printf("\t   [--sock-dir <sock dir> --sock-port <port>]\n");
    printf("\t   [--help]\n");
...
static struct option long_options[] = {
    ...
    [SOCK_PORT_OPT_INDEX]    =  {"sock-port",   required_argument,  &sock_port_set,     1},
...
parse_long_opts(int option_index, char *opt_arg)
{
    ...
    switch (option_index) {
        ...
        case SOCK_DIR_OPT_INDEX:
            ...
            break;
        case SOCK_PORT_OPT_INDEX:
            vr_netlink_port = (uint16_t) safer_strtoul(opt_arg, NULL, 0);
            break;
...
validate_options(void)
{
    ...
    if ((sock_dir_set && !sock_port_set) || (!sock_dir_set && sock_port_set)) {
         Usage();
         return;
    }
    ...
    /*
     * Reduce sum_opt by 2 so that rest of the validation logic doesn't take
     * sock_dir_set and sock_port_set into account
     */
    if (sock_dir_set) {
        sum_opt -=2;
    }
...
main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "ba:c:d:g:klm:t:T:v:p:C:DPi:s:n:",
        ...
        switch (opt) {
            ...
            case 'n':
                sock_port_set = 1;
                parse_long_opts(SOCK_PORT_OPT_INDEX, optarg);
                break;
            case 0:
