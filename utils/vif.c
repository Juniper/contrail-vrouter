...
#define RATE_NO_EVENT 0x0
#define RATE_NEXT_INTERFACE 0x1
#define RATE_PREVIOUS_INTERFACE 0x2
...
static unsigned char key_event = RATE_NO_EVENT;
...
vr_interface_req_process(void *s)
{
    ...
    if (rate_set && list_set){
        if  ((key_event & RATE_PREVIOUS_INTERFACE) && (rate_req_temp.vifr_idx -1) >  dump_marker) {
...
rate_stats(struct nl_client *cl, unsigned int vr_op)
{
    ...
    while(true) {
        while(!is_stdin_hit() || get_set){
            ...
            printf("-------------------------\n\n");
            number_interface = (terminal_size.ws_row - 6) / LISTING_NUM_OF_LINE ;
        ...
        switch (tolower(kb_input[0])) {
           ...
           case 'j':
                ...
                initialized = 2;
                key_event = RATE_PREVIOUS_INTERFACE;
                break;
