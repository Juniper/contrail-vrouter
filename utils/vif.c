...
/*
static void
vr_interface_e_per_lcore_counters_sum_print(const char *title, bool print_always,
        uint64_t *errors, uint32_t size)
{
    unsigned int i;
    uint64_t errors_sum = 0;

    for (i = 0; i < size; i++) {
        errors_sum += errors[i];
    }

    if (errors_sum || print_always) {
        vr_interface_print_head_space();
        vr_interface_core_print();
        printf("%s lcores errors:%" PRId64 "\n", title, errors_sum);
    }
