...
do
	sandesh_proto.fields[sandesh_global[i].name] = ProtoField.protocol(sandesh_global[i].abv , sandesh_global[i].name)
	object_table = sandesh_global[i].table
	for k, v in pairs(object_table)
	do
		
		sandesh_proto.fields[v.field_name] = ProtoField.none(sandesh_global[i].abv .. '.' .. v.field_name, v.field_name)
		sandesh_proto.fields[v.field_name .. '_ttype'] = ProtoField.bytes(sandesh_global[i].abv .. '.' .. v.field_name .. '.ttype', "TType")
		sandesh_proto.fields[v.field_name .. '_sr_no'] = ProtoField.uint16(sandesh_global[i].abv .. '.' .. v.field_name .. '.sr_no', "Serial Number", base.DEC)
		sandesh_proto.fields[v.field_name .. "_list_datatype"] = ProtoField.bytes(sandesh_global[i].abv .. '.' .. v.field_name .. '.list_datatype', "List Datatype")
		sandesh_proto.fields[v.field_name .. "_list_length"] = ProtoField.uint32(sandesh_global[i].abv .. '.' .. v.field_name .. '.list_length', "List Length", base.DEC)
		sandesh_proto.fields[v.field_name .. '_value'] = v.ProtoField(sandesh_global[i].abv .. '.' .. v.field_name .. '.value', "Value", v.base)
		field_extractors[v.field_name .. '_value_str'] = Field.new(sandesh_global[i].abv .. '.' .. v.field_name .. '.value') 
	end
    ...
    for j = 1, tablelength(sandesh_global[i].decode_bits) do
			...
			sandesh_proto.fields[val] = sandesh_global[i].decode_bits[j][2](val, val, base.HEX, nil, sandesh_global[i].decode_bits[j][3])
		end
	end
	end
...
-- dissects the buffer 
function object_dissector(structure, buffer, pinfo, tree, structure_table, length, structure_in_info_col, protocol_name)
    local offset = 0
    ...
    do
        ...
        if  datatype_size[TType[ttype:uint()]] == "variable" then
            if TType[ttype:uint()] == "T_LIST" then
            branch:add(f[field_name .. "_list_datatype"], element):append_text(" (" .. TType[element_in_dec] .. ")")
            end
		...
		elseif depends_on_field then
			...
			use_str = add_value_as_per_field(field_name, depends_on_field, branch, value_field, value)
        --elseif decode_bits is present then
        elseif decode_bits_table then 
			decoded_bits_branch = branch:add(f[field_name .. "_value"], value)
            use_str = add_decoded_bits_to_branch(field_name, decode_bits_table, branch, decoded_bits_branch, value, offset, data_len, field_val_info_col)
        ...
        elseif ntoh(TType[ttype:uint()]) then
                ...
                value_field = branch:add_packet_field(f[field_name .. "_value"], value, get_host_endianness())
        elseif  TType[ttype:uint()] == "T_LIST" then
               use_str = add_list(branch, value, list_size_in_hex:uint(), element_in_dec, field_name, hide, set_to_default)
        else 
			value_field = branch:add(f[field_name .. "_value"], value)
			if string.match(field_name, "ip6_u") or string.match(field_name, "ip6_l") then
      			use_str = covert_to_ipv6_format(value)
      			value_field:append_text(" (" .. ipv6_str .. ")")
    		end
        end
        append_value(field_name, field_append_value, use_str, subtree, branch, value_field, value, hide, field_val_info_col, set_to_default)
...
function sandesh_proto.dissector(buffer, pinfo, tree)	
	...
	while(offset<buffer:len()) do
		local struct_str , length = structure_name(buffer(offset, -1), tree, length_parsed , buffer:len())
		offset = offset + 1
		...
		do	
			if struct_str == sandesh_global[i].name then
				...
				length_parsed = object_dissector(struct_str, buffer(offset-1, -1), pinfo, tree, structure_table, length, structure_in_info_col, sandesh_global[i].protocol)
