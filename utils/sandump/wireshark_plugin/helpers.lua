...
function to_hide(data_len, value, show_field_when_zero)
	...
	--only nhr_encap_oif_id, nhr_vrf, nhr_id and nhr_type's zero value will be shown
...
function ntoh(ttype)
	if ttype == "T_U8" or ttype == "T_U16" or ttype == "T_U32" or ttype == "T_U64" then
		return true
	else
		return false
	end
...
function add_ttype_and_sr_number(branch, field_name, ttype, serial_number)
	...
	branch:add(f[field_name .. "_ttype"], ttype):append_text(" (" .. TType[ttype:uint()] .. ")")
...
function add_decoded_bits_to_branch(field_name, decoded_bits_lookup_table, branch, decoded_bits_branch, value, offset, datatype_size, field_val_info_col)
    ...
    for i = 1, #decoded_bits do
        local decoded_bits_buffer = ByteArray.new(string.rep('0', offset*2) .. decoded_bits[i]):tvb(decoded_bits_lookup_table["0x" .. decoded_bits[i]])
        decoded_bits_branch:add(f[decoded_bits_lookup_table["0x" .. decoded_bits[i]]], decoded_bits_buffer(decoded_bits_buffer:len()-datatype_size, datatype_size))
...
function convert_to_list_datatype(value, list_size_in_dec, element_type_size, ttype)
	arr = {}
    if value ~= 0  then  
        local i = 0
...
-- adds int type lists to the branch (except byte lists)
function add_list(branch, value, list_size_in_dec, element_in_dec, field_name, hide, set_to_default)
	local str = ""
	if value ~= 0 then
		f = sandesh_proto.fields
		element_type_size = datatype_size[TType[element_in_dec]]
		arr = convert_to_list_datatype(value, list_size_in_dec, element_type_size, TType[element_in_dec])
		...
		if set_to_default and tostring(value) == set_to_default.buffer then
			...
			if TType[element_in_dec] == "T_BYTE" and string.match(field_name, "name") then
				table_str = {}
					 ...
					 do 
						if tostring(value(i,1)) == "00"  then
							...
							table.insert(table_str, string.char(tonumber(tostring(value(i,1)),16)))
						end
					 end
				str = convert_arr_to_str(table_str, "", false)	
				val_branch:append_text(" (".. str .. ")")
				branch:append_text(": " .. str)
			elseif TType[element_in_dec] == "T_BYTE" and string.match(field_name, "mac") then
				str = convert_arr_to_str(arr, ":", false)
				branch:append_text(": " .. str) 
			else
			branch:append_text(": " .. "[" .. str .. "]")
...
function append_value(field_name, field_append_value , use_str, subtree, branch, value_field, value, hide, field_val_info_col, set_to_default)
	if field_append_value and not(hide) then
		for k, v in pairs(field_append_value) do
			...
			if k == "branch" and not(added_to_branch) then
				 ...
				 added_to_branch = true
				if field_val_info_col then
					info_col[field_name] = {prepend = field_val_info_col.prepend, value = v.value(tostring(val_str))} 
				end
			end
			if k == "subtree" then
	...
	if not(added_to_branch) then
		...
		added_to_branch = true
		if field_val_info_col then
			info_col[field_name] = {prepend = field_val_info_col.prepend, value = tostring(val_str)}
		end
	end
end
...
function add_value_as_per_field(field_name, depends_on_field, branch, value_field, value, field_val_info_col)
	if string.match(depends_on_field, "family") then
	val_str = get_field_value(field_extractors[depends_on_field  .. '_value_str'])	
	local str = ""
	if tostring(val_str) == "1" or tostring(val_str) == "2" or tostring(val_str) == "7" then
			-- ipv4
...
-- updates info column 
function update_proto_info_col(pinfo, structure, structure_in_info_col, protocol_name)

    local pkt_info = pinfo.cols.info
	str = ""
	for k, v in pairs(info_col) do
		if v.value == "nil" or v.value == nil or (string.match(tostring(pkt_info), "Response") and string.match(v.prepend, "Operation: ")) then
			--do nothing
		else
	...
	elseif  string.match(tostring(pkt_info), "Operation") or string.match(tostring(pkt_info), "ID") or string.match(tostring(pkt_info), "Vrf")then
		if not(string.match(tostring(pkt_info), str)) then
				if not(string.match(tostring(pkt_info), structure)) then
					if string.match(tostring(pkt_info), "Response:") and (string.match(tostring(pkt_info), "Multiple") or not(string.match(tostring(pinfo.cols.protocol), protocol_name)))then
					 	sub_str = string.sub(tostring(pkt_info), 1, 20)
						print(tostring(pkt_info))
					...
					elseif (not(string.match(tostring(pkt_info), "Response:")) and string.match(tostring(pkt_info), "Multiple")) or tablelength(structure_in_info_col)>1 then
		...
		if not(string.match(tostring(pkt_info), structure)) then
        	if tostring(pkt_info) == "" or string.match(tostring(pkt_info), "Response:") then
