...
int vr_set_log_request(struct nl_client *cl, unsigned int router_id)
{
    ...
    req.vo_resize_buf_size = sizeof(int)*VR_NUM_MODS;
    ...
    for(i=0;i<VR_NUM_MODS;i++) {
	    ...
	    req.vo_log_mod_level[i] = log_ctrl[i].level;
            if(log_ctrl[i].size != 0) {
                req.vo_resize_buf[i] = 1;
                req.vo_log_mod_len[i] = log_ctrl[i].size*VR_LOG_ENTRY_LEN;
            }
            else {
                req.vo_log_mod_len[i] = -1;
                req.vo_resize_buf[i] = 0;
            }
        }
        else{
