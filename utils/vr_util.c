...
int vr_set_log_options_request(struct nl_client *cl, unsigned int router_id, int module)
{
    ...
    req.vo_log_mod_level_size = sizeof(int)*VR_NUM_MODS;
    req.vo_log_mod_len_size = 0;
    req.vo_log_mod_level = (int *) malloc(req.vo_log_mod_level_size);
    req.vo_module = module;
    req.vo_log_mod_type = vr_logger.log_module[module].console;
    req.vo_log_mod_level[module] = vr_logger.log_module[module].level;
...
int vr_set_log_sizes_request(struct nl_client *cl, unsigned int router_id, int module, int level)
{
    ...
    memset(&req, 0, sizeof(req));
    req.h_op = SANDESH_OP_ADD;
    req.vo_log_mod_len_size = sizeof(unsigned int) * (VR_NUM_MODS * VR_NUM_LEVELS);
    req.vo_log_mod_level_size = 0;
    req.vo_log_mod_len = (unsigned int *) malloc(req.vo_log_mod_len_size);
    req.vo_module = module;
    req.vo_level = level;
    req.vo_log_mod_type = vr_logger.log_module[module].console;
    req.vo_log_mod_len[module * VR_NUM_LEVELS + level] =
    vr_logger.log_module[module].level_info[level].num_records*VR_LOG_ENTRY_LEN;
