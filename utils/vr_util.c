...
vr_print_drop_stats(vr_drop_stats_req *stats, int core)
{
    ...
    PRINT_DROP_STAT("Fragment Stale entry drops",stats->vds_frag_stale_err);
    PRINT_DROP_STAT("Fragment Non head pkt drops",stats->vds_frag_non_head_err);
    PRINT_DROP_STAT("Fragment per cpu max errors",stats->vds_frag_max_per_cpu_err);
    PRINT_DROP_STAT("Fragment all cpu core max errors",stats->vds_frag_max_cpu_core_err);
    PRINT_DROP_STAT("Fragment mem alloc fail",stats->vds_frag_mem_alloc_err);
    PRINT_DROP_STAT("Fragment Enqueue fail", stats->vds_frag_enqueue_err);
    PRINT_DROP_STAT("Fragment DPDK CPU error", stats->vds_frag_cpu_err);
    PRINT_DROP_STAT("Fragment wrong cpu select error",stats->vds_frag_cpu_exceed_err);
                          ...
                          uint8_t show_pkt_drop_type)
{
   ...
   printf("Epoch Time: %Id ", pkt_log_utils[i].timestamp);
    ...
    {
        if (inet_ntoa(pkt_log_utils[i].src.ipv4) != NULL )
        {
            printf("Src IP: %s  ", inet_ntoa(pkt_log_utils[i].src.ipv4));
        }
        if ( inet_ntoa(pkt_log_utils[i].dst.ipv4) != NULL) 
        {
