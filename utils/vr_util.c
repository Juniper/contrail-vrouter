...
int vr_set_log_options_request(struct nl_client *cl, unsigned int router_id, int module)
{
    ...
    req.vo_log_mod_level[module] = log_ctrl[module].level;
...
int vr_set_log_sizes_request(struct nl_client *cl, unsigned int router_id, int module, int level)
{
    ...
    req.vo_resize_buf_size = sizeof(int)*(VR_NUM_MODS*VR_NUM_LEVELS);
    req.vo_log_mod_len = (unsigned int *) malloc(req.vo_log_mod_len_size);
    req.vo_resize_buf = (int *) malloc(req.vo_resize_buf_size);
    req.vo_module = module;
    req.vo_level = level;
    req.vo_resize_buf[module*VR_NUM_LEVELS+level] = 1;
    req.vo_log_mod_len[module*VR_NUM_LEVELS+level] = log_ctrl[module].entries[level]*VR_LOG_ENTRY_LEN;
