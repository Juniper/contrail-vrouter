...
print_module_log_status(vrouter_ops *req)
{
    ...
    for(i = 0; i < VR_NUM_MODS; i++) {
        level = req->vo_log_mod_level[i]&LOG_LEVEL_MASK;
        if((req->vo_log_mod_level[i]&LOG_CONSOLE_MASK)>>3 == 0) log_type = "buffer";
        else log_type = "console";
        printf("%-22s %-22s %-22s %-22d\n", mod_id_to_name[i],\
        level_id_to_name[level], log_type, req->vo_log_mod_len[i*VR_NUM_LEVELS+level]*VR_LOG_ENTRY_LEN);
    }
    printf("Use --ext_info for extended vrouter logging info\n");
}

static void
print_module_ext_log_status(vrouter_ops *req)
{
    int i, j, level;
    char *log_type;
    ...
    for(i=0;i<VR_NUM_MODS;i++) {
        ...
        req->vo_log_mod_len[i*VR_NUM_LEVELS+1]*VR_LOG_ENTRY_LEN);
        for(j=2;j<VR_NUM_LEVELS;j++) {
            printf("%58s %22d\n", level_id_to_name[j], req->vo_log_mod_len[i*VR_NUM_LEVELS+j]*VR_LOG_ENTRY_LEN);
        }
        printf("\n");
    }
}
...
_vr_log_response_process(void *s_req)
{
     ...
     while(strlen(req->vdl_vr_log + i) != 0 && i < MAX_READ) {
         ...
         printf("%s", req->vdl_vr_log + i);
         i += (VR_LOG_ENTRY_LEN);
     }
       index = req->vdl_log_idx;
       cur_index = req->vdl_cur_idx;
       if(index == cur_index) return 0;
       vr_get_log_request(cl, 0, retrieve_log_module, retrieve_log_level, index, cur_index);
...
vr_vrouter_op(struct nl_client *cl)
{
    ...
    case SANDESH_OP_GET:
        if(opt[GET_VROUTER_LOG_INDEX]) {
            ret = vr_get_log_request(cl, 0, retrieve_log_module, retrieve_log_level, 0, -1);
	    }
        else
    ...
    case SANDESH_OP_ADD:
        if(opt[SET_VROUTER_LOG_OPTIONS_INDEX]) {
            ...
            ret = vr_set_log_sizes_request(cl, 0, retrieve_log_module, retrieve_log_level);
        }
        else if(opt[CLEAR_VROUTER_LOG_INDEX]) {
            ret = vr_clear_log_request(cl, 0, retrieve_log_module, retrieve_log_level);
        }
...
int vr_level_name_to_id(char *s)
{
    ...
    if(strcmp("none", s) == 0) return vr_none;
...
void parse_command_line_set(char *opt_arg, bool size_opt)
{
    ...
    if(j > 1 && strlen(args[1]) != 0) {
        ...
        SET_MOD_CLI(module);
        SET_MOD_LOG_LEVEL(module, level);
    }
    else {
        SET_MOD_CLI(module);
        SET_MOD_LOG_LEVEL(module, vr_info);
    }
    if(size_opt == 0) {
        if(j > 2 && strlen(args[2]) != 0) {
            if(strcmp(args[2], "con") == 0) SET_CONSOLE_LOG(module);
            else if(strcmp(args[2], "buf") == 0) SET_BUFFER_LOG(module);
            else {
        ...
        if(j > 2 && strlen(args[2]) != 0) {
            SET_MOD_LOG_ENTRIES(module, level, atoi(args[2]));
        }
        else {
            SET_MOD_LOG_ENTRIES(module, level, VR_LOG_DEFAULT_ENTRIES);
        }
    }
