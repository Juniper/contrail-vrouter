...
static void drop_stats_log_req_process(void *s_req) {
    ...
    if(stats->vds_drop_stats_support == 1)
    {
         ...
         * and request the data based on the index
         * */
...
/*            else if( stats->vds_stats_index <(stats->vds_drop_stats_max_log_buffer_size - VR_DROP_STATS_MAX_ALLOWED_BUFFER_SIZE))
            {
    ...
    else
    {
        ...
        printf("You can enable by providing \"options vrouter vr_config_drop_stats_log_buffer_support=1\" in /etc/modprobe.d/vrouter.conf\n");
    }
    return;
}
...
#if (VR_DROP_STATS_LOG_BUFFER_INFRA ==STD_ON)
    printf("--log <core number>\t Show Dropstats log for a specified core.. Core number starts from 1...n \n");
#endif
    exit(-EINVAL);
}
