...
static int vr_get_pkt_drop_log(struct nl_client *cl, int core);
    ...
    if(stats->vdl_pkt_droplog_sysctl_en == 1) {
        ...
        if(stats->vdl_pkt_droplog_en == 1) {
            ...
            if(stats->vdl_pkt_droplog_repeat == 1)
                vr_get_pkt_drop_log(cl, stats->vdl_core);
...
static int vr_get_pkt_drop_log(struct nl_client *cl, int core)
{
    ...
    ret = vr_pkt_drop_log_request(cl, 0, core);
...
main(int argc, char *argv[])
{
    ...
    if ((option_index == LOG_OPT_INDEX) || (log_set == 1)) {
        ...
        vr_get_pkt_drop_log(cl, log_core);
        return 0;
    }
