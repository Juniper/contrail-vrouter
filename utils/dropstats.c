...
static int vr_get_pkt_drop_log(struct nl_client *cl, int core, unsigned int stats_cntr);
    ...
    if(stats->vdl_pkt_droplog_sysctl_en == 1) {
        ...
        if(stats->vdl_pkt_droplog_en == 1) {
            ...
            if(stats->vdl_pkt_droplog_repeat == 1)
                vr_get_pkt_drop_log(cl, stats->vdl_core, stats->vdl_pkt_droplog_stats_cntr);
            ...
            if(stats->vdl_core == 0 || log_all_cores == 1) {
                ...
                if(core < stats->vdl_max_num_cores) {
                    ...
                    vr_get_pkt_drop_log(cl, core+1, stats->vdl_pkt_droplog_stats_cntr);
                }
                else {
                    log_all_cores = 0;
                }
...
static int vr_get_pkt_drop_log(struct nl_client *cl, int core, unsigned int stats_cntr)
{
    ...
    ret = vr_pkt_drop_log_request(cl, 0, core, stats_cntr);
...
main(int argc, char *argv[])
{
    ...
    if ((option_index == LOG_OPT_INDEX) || (log_set == 1)) {
        ...
        vr_get_pkt_drop_log(cl, log_core, stats_cntr);
        return 0;
    }
