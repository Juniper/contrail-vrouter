 ...
 * client.c
 *
 * Client data strucutres procedures, which initialize, deinitialize
 * and communicate (control communication) with virtio device.
 *
...
client_vhost_ioctl_send_fds(VhostUserMsg *msg, int fd, int *fds, size_t fd_num) {
    ...
    struct iovec iov;K vRouter. More precisely, there are cases when the Cassandra tries to bind to the IP fabri
    struct msghdr msgh;
