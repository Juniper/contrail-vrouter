...
main (int argc, char **argv) {
    ...
    while(1) {
        ...
        if (tx_rx_state == S_SEND) {
            ...
            return_val_tx = tx_rx_handler.send_data->tx(tx_rx_handler.send_data->context, ( u_char *)tx_rx_handler.pkt_data, (size_t *) &(tx_rx_handler.pkt_header->len));
            tx_rx_state = S_RECV;

        } else if (tx_rx_state == S_RECV || tx_rx_state == S_RECV_TIMEOUT) {
            return_val_rx = tx_rx_handler.recv_data->rx(tx_rx_handler.recv_data->context, &tx_rx_handler.data_dest_buf, &tx_rx_handler.data_len_recv);
            ...
            if (return_val_rx == EXIT_SUCCESS ) {
                ...
                pcap_dump((u_char*) tx_rx_handler.dumper, &tx_rx_handler.pcap_hdr, (u_char *) tx_rx_handler.data_dest_buf);
