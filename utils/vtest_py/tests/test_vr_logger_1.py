...
def test_vr_logger_1(vrouter_test_fixture):
    ...
    while(True):
        ...
        line_idx = int(vt.parse_xml_field(vrlog_resp_file, "vdl_line_idx"))
        vr_log_str_arr = vr_log_str.split("\n")
        for log in vr_log_str_arr:
            if(log == '\x00'):
                continue
            comp_str = ("[error] OP:0 fam:2 rid:0 lab:0 nh:%d Err:-2 prefix:2.2.1.1.") %(id)
            assert((comp_str in log)), "Failed to get matching vr_log"
            id += 1
	cur_idx = line_idx * 100
        if (log_idx == cur_idx):
            break
    return 0
