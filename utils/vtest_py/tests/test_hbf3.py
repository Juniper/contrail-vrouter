...
def test_hbf_left_vm_to_right_vm_intra_vn(vrouter_test_fixture):

    vt = vtest("test_hbf_left_vm_to_right_vm_intra_vn")
   ...
   # Add hbf-l vif
    ...
    # send encoded packet from hbs-r and receive on vif4
    eth = Ether(src='ca:f1:00:02:40:08', dst='02:e7:03:ea:67:f1', type=0x800)
    ip = IP(version=4, ihl=5, id=1, ttl=64, proto='icmp', dst='1.1.1.5', src='1.1.1.4')
    icmp = ICMP(type=8, code=0, id=1136)
    pkt = eth/ip/icmp
    pkt.show()

    # send packet and receive on vif4
    vt.send_recv_pkt(pkt, "tap8b05a86b-36", pkt, "tape703ea67-f1")

    # Check if the packet was sent on vif3 and received at vif4
    vif = vr_interface_req()
