...
def test1(vrouter_test_fixture):
    ...
    vt = vtest("test1", vt_path, sock_dir)
    ...
    vif_list = []
    vif_list.append(vif)
    vif_resp_file = vt.send_sandesh_req(vif_list, vt.VT_RESPONSE_REQD)

    # parse the fields and validate the response
    vif_name = vt.parse_xml_field(vif_resp_file[0], "vifr_name")
    print "Got vif name ", vif_name
    assert (vif_name.find("1") != -1), "Failed to get vif name"
    vif_mtu = vt.parse_xml_field(vif_resp_file[0], "vifr_mtu")
