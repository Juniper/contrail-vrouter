...
def test_vrf_table(vrouter_test_fixture):
   ...
   # Add hbf-l vif
    ...
    vrf.vrf_rid = 0
    vrf.vrf_idx = 5
    vrf.vrf_flags = vtconst.VRF_FLAG_HBF_L_VALID
    vrf.vrf_hbfl_vif_idx = 3

    vt.send_sandesh_req(vrf)

    # Add hbf-r in vrf table
    vrf = vr_vrf_req()
    vrf.h_op = vtconst.SANDESH_OPER_ADD
    vrf.vrf_rid = 0
    vrf.vrf_idx = 5
    vrf.vrf_flags = vtconst.VRF_FLAG_HBF_R_VALID
    vrf.vrf_hbfr_vif_idx = 4

    vt.send_sandesh_req(vrf)

    # Remove hbf-r in vrf table
    vrf = vr_vrf_req()
    vrf.h_op = vtconst.SANDESH_OPER_ADD
    vrf.vrf_rid = 0
    vrf.vrf_idx = 5
