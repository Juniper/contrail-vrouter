...
def test_mirror_v6_sandesh_conf(vrouter_test_fixture):
    ...
    print os.getcwd() 
    pkt = rdpcap("./tests/pcaps/mirror_v6_src.pcap")
    pkt.show()

    exp_pkt = rdpcap("./tests/pcaps/mirror_v6_dst.pcap")
    exp_pkt.show()

    vt.send_recv_pkt(pkt, "1", exp_pkt, "3")
    ...
    assert (vif_name.find("3") != -1), "Failed to get vif name"
    vif_opackets = vt.parse_xml_field(vif_resp_file, "vifr_opackets")
    print "Received packets ", vif_opackets
    #Expecting one received packet and mirror packet, so count as 2.
    assert (vif_opackets.find("2") != -1), "Failed to receive Mirror packet"
    print "Test 1 passed"
    return 0
