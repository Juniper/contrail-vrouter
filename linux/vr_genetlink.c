...
netlink_trans_request(struct sk_buff *in_skb, struct genl_info *info)
{
    ...
    while ((response = vr_message_dequeue_response())) {
	  ...
	  nla_put(skb, NL_ATTR_VR_MESSAGE_PROTOCOL, response->vr_message_len, response->vr_message_buf);
	  genlmsg_end(skb, msg_head);

	  ret = genlmsg_multicast_allns(&vrouter_genl_family, skb, 0, 0, GFP_KERNEL);
	  if (ret)
            vr_printf("vrouter: broadcast failed (%d)\n", ret);
	}
