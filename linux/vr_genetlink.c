...
netlink_trans_request(struct sk_buff *in_skb, struct genl_info *info)
{
	...
	error = nla_put(skb, NL_ATTR_VR_MESSAGE_PROTOCOL, response->vr_message_len, response->vr_message_buf);
	if (error) {
	  continue;
	}
	genlmsg_end(skb, msg_head);

	// TODO: remove it once client discover group id
	pr_info("The multicast group ID is %u.\n", vrouter_genl_family.mcgrp_offset);
	error = genlmsg_multicast_allns(&vrouter_genl_family, skb, 0, 0, GFP_KERNEL);
	// Can be ignored since it occurs when no listeners are present
	if (error)
	  continue;
      }
