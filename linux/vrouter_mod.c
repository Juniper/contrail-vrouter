...
lh_get_udp_src_port(struct vr_packet *pkt, struct vr_forwarding_md *fmd,
                    unsigned short vrf, void *eth_hdr)
{
    struct sk_buff *skb = vp_os_packet(pkt);
    int pull_len;
    ...
    } /* else if (pkt->vp_type == VP_TYPE_IP6) { not implemented yet } */
    else {

        if (!eth_hdr)
            goto error;

        hashval = vr_hash(eth_hdr, ETH_HLEN, vr_hashrnd);
    ...
    if (ntohs(udph->udp_dport) == VR_MPLS_OVER_UDP_DST_PORT) {
        ...
        *encap_type = PKT_ENCAP_MPLS;
        ...
        if (pkt_type == PKT_MPLS_TUNNEL_L3) {
            ...
            if (pkt_type == PKT_MPLS_TUNNEL_L2_MCAST_EVPN)
                pkt->vp_flags |= VP_FLAG_MULTICAST;
        } else {
            goto unhandled;
        }
    ...
    *reason = VP_DROP_PULL;
    ...
    if (mpls_pkt) {
        ...
        if (ret == PKT_MPLS_TUNNEL_L3) {
            ...
            eth = (struct vr_eth *) (skb->head + hoff);
            if (ret == PKT_MPLS_TUNNEL_L2_MCAST_EVPN)
                pkt->vp_flags |= VP_FLAG_MULTICAST;
