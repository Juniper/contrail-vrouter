...
define dump_bridge_table
    ...
    set $temp_vrf = $arg1
    set $max_index = $bridge_table.ht_hentries + $bridge_table.ht_oentries - 1
    dump_bridge_range_internal 0 $max_index $temp_vrf
end

#arg0:start_index, arg1:end_index
define dump_bridge_range
    set $temp_vrf = -1
    dump_bridge_range_internal $arg0 $arg1 $temp_vrf
end

#arg0:start_index, arg1:end_index, arg2:vrf_id
define dump_bridge_range_internal
    ...
    if($bridge_table)
        ...
        while(($i < $bridge_table.ht_hentries) && ($i<$arg1))
            dump_bridge_internal $temp_table_ptr $i $arg2
            set $i = $i + 1
        ...
        while(($j<$bridge_table.ht_oentries) && ($j<$k))
            dump_bridge_internal $temp_table_ptr $j $arg2
            set $j = $j + 1
            set $temp_table_ptr = $temp_table_ptr + $bridge_table.ht_entry_size
        end
        end
...
define dump_bridge_internal
    if($arg0 != -1)
        ...
        if(($cur_bridge.be_flags & 1)&&(($cur_bridge.be_key.be_vrf_id == $arg2)||$arg2 == -1))
