...
define dump_nh_index
	set $bt_ret_val = -1
	get_index_addr_btable router.vr_nexthops $arg0
	if($bt_ret_val != -1)
		set $cur_nh = *(struct vr_nexthop **)$bt_ret_val
		if($cur_nh != 0)
			printf "ID:%-9dType:", $arg0
			get_nh_type $cur_nh.nh_type
			printf " Fmly:"
			get_nh_family $cur_nh.nh_family
			printf " Users:%u Rid:%u Vrf:%d\n            Router:0x%x ", $cur_nh.nh_users, $cur_nh.nh_rid, $cur_nh.nh_vrf, $cur_nh.nh_router
			get_nh_flags $cur_nh.nh_flags
...
#Encap
	if ($arg0 == 2)
		get_nh_encap_data $arg3.nh_encap
	end
#Tunnel
	if ($arg0 == 3)
		if (($arg2 >> 3) & 1)
			get_nh_gre_tunnel_data $arg3.nh_gre_tun
		end
                if (($arg2 >> 7) & 1)
                        get_nh_vxlan_tunnel_data $arg3.nh_vxlan_tun
                end
                if (($arg2 >> 19) & 1)
                        get_nh_pbb_tunnel_data $arg3.nh_pbb_tun
                end
                if (($arg2 >> 4) & 1)
			if ($arg1 == 2)
                        	get_nh_udp_tunnel_data $arg3.nh_udp_tun 0
			end
			if ($arg1 == 10)
				get_nh_udp6_tunnel_data $arg3.nh_udp_tun6
			end
                end
		if (($arg2 >> 6) & 1)
			get_nh_udp_tunnel_data $arg3.nh_udp_tun 1
		end
	end
#Composite
	if ($arg0 == 6)
...
define get_nh_encap_data
	printf "\n            Len:%u EncapFmly:%u", $arg0.encap_len, $arg0.encap_family
end

#arg0:nh_gre_tun of cur_nh
define get_nh_gre_tunnel_data
	printf "\n            ScrIP:"
...
define get_nh_udp6_tunnel_data
	if ($arg0.tun_sip6 != 0)
		printf "\n            SrcIPv6:", 
	...
	printf "\n            SrcPort:%u DstPort:%u Len:%u", $arg0.tun_sport6, $arg0.tun_dport6, $arg0.tun_encap_len
end
...
define get_nh_comp_data
	...
	get_nh_comp_config_hash $arg0.ecmp_config_hash
        while ($cnh_count < $arg0.cnt)
                ...
                printf "\n            Sub NH:%u Label:%d EcmpIndex:%d ", $cur_cnh.cnh.nh_id, $cur_cnh.cnh_label, $cur_cnh.cnh_ecmp_index
...
define get_ipv4
	printf "%d.%d.%d.%d ", ($arg0 & 0xff), ($arg0 >> 8) & 0xff, ($arg0 >> 16) & 0xff, ($arg0 >> 24)  & 0xff
end
